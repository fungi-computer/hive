...
use crate::structure_geometry::{Face, FaceAxis, StaticGeometry, StaticInstance};
...
fn door_and_upper_vent_keep_frame_support_while_opening_only_the_interval() {
    for (bottom, height) in [(0u8, 2u8), (2u8, 1u8)] {
        ...
        let closed = StaticGeometry::new(bounds(), vec![StaticInstance::ApertureWall {
            id: "aperture".into(), edge: Face { cell: base, axis: FaceAxis::X }, height: 5, opening_bottom: bottom, opening_height: height, open: false,
        }]).unwrap();
        let open = StaticGeometry::new(bounds(), vec![StaticInstance::ApertureWall {
            id: "aperture".into(), edge: Face { cell: base, axis: FaceAxis::X }, height: 5, opening_bottom: bottom, opening_height: height, open: true,
        }]).unwrap();
        let closed_projection = closed.projection().unwrap();
        let open_projection = open.projection().unwrap();
        assert!(closed_projection.is_face_sealed(Face { cell: Cell { y: i32::from(bottom), ..base }, axis: FaceAxis::X }));
        assert!(!open_projection.is_face_sealed(Face { cell: Cell { y: i32::from(bottom), ..base }, axis: FaceAxis::X }));
        assert!(open_projection.is_face_sealed(Face { cell: Cell { y: 4, ..base }, axis: FaceAxis::X }));
    }
}
...
fn rooted_aperture_wall_is_supported_by_terrain_anchor() {
    ...
    let geometry = StaticGeometry::new(bounds(), vec![StaticInstance::ApertureWall {
        id: "door".into(), edge: Face { cell: base, axis: FaceAxis::X }, height: 4, opening_bottom: 0, opening_height: 2, open: true,
    }]).unwrap();
...
fn constructed_aperture() -> (Kernel, Cell, Point) {
    ...
    kernel.load(&serde_json::json!({"format":"hive-game","version":3,"game":"aperture-laws","components":[],"materialCatalog":[],"initial":[
