...
pub fn validate_definition(definition: &ProcessDefinition) -> Result<(), String> {
    ...
    for stage in &definition.stages {
        if !stage.ticks.is_finite() || stage.ticks <= 0.0 || stage.ticks > 86_400.0 || !valid_id(&stage.operation) { return Err("invalid staged-process stage".into()); }
    }
    Ok(())
}
...
pub fn admit(definition: &ProcessDefinition, binding: &ProcessBinding, id: String, tick: u64) -> Result<StagedProcess, String> {
    ...
    Ok(StagedProcess { version: CURRENT_VERSION, id, definition: definition.id.clone(), definition_version: definition.version, binding: binding.id.clone(), station: binding.station.clone(), stage: 0, progress: 0.0, entered_tick: tick, status: ProcessStatus::Active })
}
...
pub fn attend(process: &mut StagedProcess, definition: &ProcessDefinition, delta: f64) -> Result<Advance, String> {
    ...
    if !delta.is_finite() || delta < 0.0 { return Err("invalid staged-process work delta".into()); }
    process.progress = (process.progress + delta).min(stage.ticks);
    if process.progress + f64::EPSILON < stage.ticks { return Ok(Advance::Pending); }
    Ok(Advance::Transition(token(process, definition)))
}

pub fn advance_unattended(process: &mut StagedProcess, definition: &ProcessDefinition, tick: u64, delta: f64) -> Result<Advance, String> {
    validate_process(process, definition)?;
    if process.status == ProcessStatus::Complete { return Ok(Advance::Complete); }
    let stage = &definition.stages[process.stage as usize];
    if stage.mode != StageMode::Unattended || tick <= process.entered_tick { return Ok(Advance::Pending); }
    if !delta.is_finite() || delta < 0.0 { return Err("invalid staged-process world delta".into()); }
    process.progress = (process.progress + delta).min(stage.ticks);
...
pub fn commit_transition(process: &mut StagedProcess, definition: &ProcessDefinition, transition: &TransitionToken, tick: u64) -> Result<Advance, String> {
    ...
    if process.progress + f64::EPSILON < definition.stages[process.stage as usize].ticks { return Err("staged-process transition is early".into()); }
...
pub fn validate_process(process: &StagedProcess, definition: &ProcessDefinition) -> Result<(), String> {
    validate_definition(definition)?;
    if process.version != CURRENT_VERSION || !valid_id(&process.id) || process.definition != definition.id || process.definition_version != definition.version || !valid_id(&process.binding) || !valid_id(&process.station) || !process.progress.is_finite() || process.progress < 0.0 || usize::from(process.stage) >= definition.stages.len() && process.status != ProcessStatus::Complete { return Err("invalid staged-process record".into()); }
...
mod tests {
    ...
    #[test] fn unattended_is_tick_gated_and_reloadable() { let (d,b)=fixture(); let mut p=admit(&d,&b,"process.1".into(),4).unwrap(); let t=match attend(&mut p,&d,2.0).unwrap(){Advance::Transition(t)=>t,_=>panic!()}; commit_transition(&mut p,&d,&t,6).unwrap(); assert_eq!(advance_unattended(&mut p,&d,6,1.0).unwrap(),Advance::Pending); assert_eq!(advance_unattended(&mut p,&d,8,1.0).unwrap(),Advance::Pending); let saved=serde_json::to_string(&p).unwrap(); let restored:StagedProcess=serde_json::from_str(&saved).unwrap(); assert_eq!(restored,p); }
    #[test] fn cancellation_only_releases_before_first_transition() { let (d,b)=fixture(); let mut p=admit(&d,&b,"process.1".into(),0).unwrap(); assert!(cancel(&p).is_ok()); attend(&mut p,&d,1.0).unwrap(); assert!(cancel(&p).is_err()); }
