...
#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields, rename_all = "camelCase")]
struct MaterialVolumeInput { kind: String, unit_volume: u32 }
#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields, rename_all = "camelCase")]
...
fn prepare_definition_mode(
    ...
    let mut unit_volume = BTreeMap::new();
    for material in definition.material_volumes {
        if !crate::components::valid_id(&material.kind) || material.unit_volume == 0 || unit_volume.insert(material.kind, material.unit_volume).is_some() { return Err("invalid material unit volume".into()); }
    }
    let material_handling = crate::material_handling::Catalog::new(unit_volume);
...
pub(crate) mod tests {
    ...
    fn rejects_oversized_or_duplicate_content() {
        ...
        let mut duplicate: serde_json::Value = serde_json::from_str(&fixture("duplicate-material")).unwrap();
        let repeated = duplicate["materials"][0].clone();
        duplicate["materials"].as_array_mut().unwrap().push(repeated);
        let error = match build_from_json(&duplicate.to_string()) {
    ...
    fn initial_surface_placements_use_generated_surface_and_survive_restore() {
        ...
        let move_output: serde_json::Value = serde_json::from_str(&kernel.advance_json(&format!(r#"{{"delta":0,"writes":[],"actions":[{{"scope":{{"kind":"host"}},"request":{{"kind":"move","entity":"actor","destination":{{"x":2,"y":{destination_y},"z":0,"frame":null}}}}}}]}}"#)).unwrap()).unwrap();
