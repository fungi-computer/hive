...
impl Kernel {
    ...
    fn plan_construction(&mut self, catalog: String, site: String, x: i64, y: i32, z: i64, orientation: crate::structure_geometry::Cardinal, contact: Point) -> Result<()> {
        if !crate::components::valid_id(&site) || self.known.contains(&site) { return Err("invalid or duplicate construction site".into()); }
    ...
    fn attend_construction(&mut self, worker: &str, site: &str) -> Result<()> {
        ...
        if self.ecs.get::<Body>(worker_entity).is_none() || self.ecs.get::<Container>(worker_entity).is_none() || self.ecs.get::<Support>(worker_entity).is_some()
    ...
    fn complete_construction(&mut self, site_id: &str, state: &ConstructionSite) -> Result<bool> {
        ...
        let prepared = {
            ...
            match environment.world.prepare_structures(instances) {
                ...
                Err(reason) if reason == "structure overlaps solid terrain" => return Ok(false),
