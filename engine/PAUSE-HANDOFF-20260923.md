...
The old Hive Vite preview and its `trycloudflare.com` tunnel were stopped. Two other `cloudflared` processes belong to broader Botanical infrastructure and were left running. I did not change `hive-public-engine-demo`, the existing game backend, or any Botanical DOs. Same-day Cloudflare DO analytics returned no invocation group for `hive-public-engine-demo` at this check, but analytics can lag and is not a billing ledger. The deployed namespace/storage may still have a small storage charge; parking prevents new test work and lets scheduled alarms retire without deleting data. Review actual Cloudflare billing if a precise cost total is needed.

## Safe restart point

Start from the accepted integration branch and the two preserved lanes. First resolve the visual-projection bound as an ownership/budget issue, integrate and prove the changed-record cut, and then run a short same-build local/workerd/hosted check before attempting the frozen ten-minute qualification. Redeploy the normal separate backend only when intentionally resuming hosted testing. The existing static preview URL will remain visible, but its backend is parked.
