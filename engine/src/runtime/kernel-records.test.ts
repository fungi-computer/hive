...
const entity = JSON.stringify({ format: "hive-kernel", version: 17, revision: 7, time: 1.5, scene: { format: "hive-game", version: 3, game: "colony", components: [], materialCatalog: [], initial: [] }, next_work_generation: 1, next_party_sequence: 1, party_bindings: [], work_attempts: [] });
...
test("current native entity version round trips and version 16 is rejected", () => {
  ...
  assert.equal(readEntityVersion(current), 17);
  const old = { ...current, records: current.records.map((record) => record.key.startsWith("kernel/entities/")
    ? { ...record, bytes: new TextEncoder().encode(entity.replace('"version":17', '"version":16')) }
    : record) };
