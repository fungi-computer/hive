...
function station(session: GameSession) {
  const site = session.query(query(ConstructionSite)).find(row => row.get(ConstructionSite).catalog === "brew-station" && row.get(ConstructionSite).phase === "finished");
  assert(site, "brew station must be built");
  return { station: site.id, hearth: entity(`${site.id}:hearth`) };
}
function buildStation(session: GameSession) {
  session.command("build", { catalog: "brew-station", target: { cell: [1, 13, -1] }, orientation: "north" });
  for (let i = 0; i < 240; i++) { session.step(0.25); if (session.query(query(ConstructionSite)).some(row => row.get(ConstructionSite).catalog === "brew-station" && row.get(ConstructionSite).phase === "finished")) return station(session); }
  throw new Error("brew station did not finish");
}
function work(session: GameSession, hearth: string) {
...
function until(session: GameSession, predicate: () => boolean, label: string, hearth?: string) {
  for (let i = 0; i < 180 && !predicate(); i++) session.step(0.25);
  assert(predicate(), label + (hearth ? ": " + JSON.stringify(work(session, hearth)) : ""));
}
...
test("station request shares supply and assignment, commits one burn, and restores its pending result", () => fixture((session, port) => {
  ...
  session.command("lightHearth", { station: selected.station }); // same station intent, no duplicate work
  session.step(0);
  assert.equal(work(session, selected.hearth).phase, "queued");
  assert.equal(work(session, selected.hearth).actor, null, "missing fuel must not claim a worker");
  until(session, () => work(session, selected.hearth).phase === "submitting", "ordinary workers must supply and light requested station");
  ...
  session.step(0.25);
  assert.equal(work(session, selected.hearth).phase, "complete");
  assert.equal(work(session, selected.hearth).actor, null);
...
test("cancelled lighting releases intent and never debits fuel", () => fixture((session) => {
  ...
  session.command("lightHearth", { station: selected.station });
  session.command("cancelIgnition", { station: selected.station });
  session.step(0);
  for (let i = 0; i < 24; i++) session.step(0.25);
  assert.equal(work(session, selected.hearth).phase, "idle");
  assert.equal(work(session, selected.hearth).actor, null);
...
test("manual takeover releases ignition attendance without taking back the player's worker", () => fixture((session) => {
  const selected = buildStation(session);
  session.command("lightHearth", { station: selected.station });
  until(session, () => work(session, selected.hearth).phase === "approaching", "station must receive ordinary worker assignment");
  const actor = work(session, selected.hearth).actor!;
...
test("refused repeat ignition releases its worker and cannot debit an already burning station", () => fixture((session) => {
  ...
  const before = wood(session);
  session.command("lightHearth", { station: selected.station });
  until(session, () => work(session, selected.hearth).phase === "complete", "first ignition must finish");
  session.command("lightHearth", { station: selected.station });
  until(session, () => work(session, selected.hearth).phase === "blocked", "native active-burn refusal must settle the job");
  assert.match(work(session, selected.hearth).reason, /already has a paid emission/);
  assert.equal(work(session, selected.hearth).actor, null);
