...
test("rectangular digging leaves an occupied tile waiting and resumes after its guest moves", () => {
  ...
  try {
    ...
    const waiting = orders[0].get(ExcavationOrder);
    assert.equal(port.workAttempts([orders[0].id]).length, 0, "waiting must release the worker");
    ...
    for (let i = 0; i < 240 && session.query(query(ExcavationOrder)).length; i++) session.step(0.1);
    assert.equal(session.query(query(ExcavationOrder)).length, 0, "last cut resumes when guest leaves");
