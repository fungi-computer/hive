...
export function installWorkerRuntime(
  scope: {
    onmessage: ((event: MessageEvent<WorkerCommand | WorkerPlacementCommand | WorkerTerrainChunksCommand>) => void) | null;
...
export async function bootGeneratedWorker(
  ...
  return installWorkerRuntime(scope, createKernel, {
    pirates: piratesPack,
    // This entry only backs ?runtime=local. The Durable Object host composes
    // colonyServerPack with an authenticated scope in region-program.ts.
    colony: colonyPack,
