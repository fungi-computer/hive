...
function checkedImpact(value: unknown): Impact {
  ...
  for (const field of ["id", "sourceId", "targetId"])
    if (
      typeof impact[field] !== "string" ||
      impact[field].length === 0 ||
      impact[field].length > 160
    )
      throw new Error("invalid physical impact identity");
  if (
    impact.projectileId !== undefined &&
  ...
  if (
    typeof impact.sequence !== "number" ||
    !Number.isSafeInteger(impact.sequence) ||
    impact.sequence < 1 ||
    typeof impact.time !== "number" ||
    !Number.isFinite(impact.time) ||
    impact.time < 0 ||
    !finiteVec3(impact.point) ||
    !finiteVec3(impact.normal) ||
    !finiteVec3(impact.velocity)
  )
...
export class GameSession {
  ...
  constructor(options: SessionOptions) {
    ...
    for (const system of this.pack.systems)
      for (const definition of system.writes) {
        const owner = writeOwners.get(definition.id);
        if (owner !== undefined && owner !== system.id)
          throw new Error(
            `duplicate system write authority for ${definition.id}: ${owner} and ${system.id}`,
          );
  ...
  start(): void {
    ...
    this.impactFrontiers = new Map(
      this.pack.systems
        .filter((system) => system.consumesImpacts)
        .map((system) => [system.id, null]),
    );
  ...
  waterContacts() {
    this.ensureLive();
    return this.port.waterContacts();
  }
  ...
  command(name: string, input: unknown): void {
    ...
    if (
      ...
      merged.length > 128 ||
      this.pendingCreates.length + creates.length > MAX_AUTHORED_RECORDS ||
  ...
  private validateAuthoredEdits(
    ...
    const inspectValue = (name: string, value: unknown) => {
      ...
      for (const [field, kind] of Object.entries(definition.fields)) {
        ...
        if (
          typeof item === "string" &&
          new TextEncoder().encode(item).length > 4096
        )
    ...
    for (const record of allCreates) {
      ...
      if (
        ...
        Object.keys(record.components).length === 0 ||
        Object.keys(record.components).length > 32
      )
    ...
    for (const record of creates)
      for (const name of Object.keys(record.components))
        if (!permitted.has(name as ComponentId))
    ...
    const ids = [...requested];
    const incomingIds =
      incoming && new Set(incoming.map((record) => record.id));
    for (let offset = 0; offset < ids.length; offset += 128) {
      const batch = ids.slice(offset, offset + 128);
      const membership = incomingIds
        ? batch.map((id) => incomingIds.has(id))
        : this.port.entityMembership(batch);
      if (membership.length !== batch.length)
        throw new Error("invalid entity membership result");
      batch.forEach((id, index) => {
        if (membership[index]) known.add(id);
      });
    }
    ...
    if (removes.length) {
      ...
      for (const definition of this.pack.components) {
        const rows = incoming
          ? incoming
              .filter((record) =>
                Object.hasOwn(record.components, definition.id),
              )
  ...
  private validateWrites(
    ...
    for (let offset = 0; offset < referencedIds.length; offset += 128) {
      ...
      batch.forEach((id, index) =>
        referenceMembership.set(id, membership[index] === true),
      );
    }
  ...
  private queryOverlay<T extends object>(
    ...
    const createdRows = creates
      ...
      .map((record) => ({
        id: record.id,
        get: <V extends object>(definition: ComponentDefinition<V>) =>
          structuredClone(record.components[definition.id]) as V,
      }));
  ...
  step(delta: number): readonly ActionResult[] {
    ...
    try {
      ...
      const context: WriteContext = {
        ...
        waterContacts: () => this.port.waterContacts(),
        routeCosts: (requests) => {
          requireRouteReads();
      ...
      let previousSequence =
        this.pendingImpacts.at(-1)?.sequence ?? this.impactHighWater;
      for (const impact of incoming) {
        if (
          seen.has(impact.sequence) ||
          impact.sequence <= this.impactHighWater
        )
          throw new Error("duplicate physical impact sequence");
        if (
          impact.time < this.now - 1e-9 ||
          impact.time > this.now + delta + 1e-9
        )
  ...
  restore(snapshot: SessionSnapshot): void {
    ...
    if (
      ...
      !Array.isArray(snapshot.pendingCreates) ||
      snapshot.pendingCreates.length > MAX_AUTHORED_RECORDS ||
      !Array.isArray(snapshot.pendingRemoves) ||
      snapshot.pendingRemoves.length > MAX_AUTHORED_REMOVES ||
    ...
    if (
      Boolean(expectedEnvironment) !== Boolean(savedEnvironment) ||
      (expectedEnvironment &&
        savedEnvironment &&
        (expectedEnvironment.byteLength !== savedEnvironment.byteLength ||
    ...
    for (const impact of pendingImpacts) {
      ...
      if (
        impactSequences.has(impact.sequence) ||
        impact.sequence <= previousSequence ||
    ...
    for (const frontier of snapshot.impactFrontiers) {
      if (
        typeof frontier.system !== "string" ||
        !expectedConsumers.has(frontier.system) ||
        frontiers.has(frontier.system) ||
        (frontier.sequence !== null &&
          (typeof frontier.sequence !== "number" ||
            !Number.isSafeInteger(frontier.sequence) ||
            frontier.sequence < 0 ||
            frontier.sequence > snapshot.impactHighWater))
      )
    ...
    const expected = this.pack.systems
      .map(
        (system) =>
          `${system.id}@${system.version}:${system.consumesImpacts === true}`,
      )
      .join(",");
