...
class TestPort implements KernelPort {
  ...
  ): import("../contracts").AtmosphereSamples {
    return {
  ...
  ): readonly import("../contracts").ConstructionReadiness[] {
    return sites.map((site) => ({ site, status: "ready" }));
  }
  constructionAccess(
    sites: readonly import("../contracts").EntityId[],
  ...
  terrainMaterials(
    _cells: readonly [number, number, number][],
  ): readonly number[] {
    return [];
  }
  terrainSurfaces(_columns: readonly [number, number][]): readonly null[] {
    return [];
  }
  structureSurfaces(
    _columns: readonly [number, number][],
  ): readonly (readonly [])[] {
    return [];
  }
  terrainChanges(
    _sinceRevision: number,
  ): import("../contracts").TerrainChangeSet {
    return { kind: "changed-columns", revision: this.revision, columns: [] };
  }
  entityMembership(
    ids: readonly import("../contracts").EntityId[],
  ): readonly boolean[] {
  ...
  query<T extends object>(_spec: QuerySpec<T>): readonly QueryRow<T>[] {
    ...
    return _spec.components.every((component) =>
      ...
      ? [
          {
            id: entity("actor"),
            get: <V extends object>(_definition: ComponentDefinition<V>) =>
              ({ value: 0 }) as V,
          },
        ]
      : [];
  ...
  advance(
    ...
    const state = JSON.parse(this.entityJson) as {
      revision: number;
      time: number;
      impactQueue: Impact[];
    };
  ...
  snapshot(): KernelSnapshot {
    ...
    const definition = root.scene; root.scene = null;
    ...
    return {
      ...
      records: [
        ...
        { key: "kernel/state/definition", bytes: new TextEncoder().encode(JSON.stringify(definition)) },
...
test("direct input admission is bounded, strict, and detached", () => {
  ...
  const batched = Array.from({ length: 50 }, (_, i) => ({
    sequence: i + 1,
    x: 1,
    z: 0,
  }));
  assert.equal(
    checkedAction({ ...action, inputs: batched }).kind,
    "direct-input",
  );
  assert.throws(
    () => checkedAction({ ...action, inputs: [batched[0], batched[2]] }),
    /invalid action/,
  );
  assert.throws(
    () =>
      checkedAction({
        ...action,
        inputs: [...batched, { sequence: 51, x: 1, z: 0 }],
      }),
    /invalid action/,
  );
  assert.throws(
    () => checkedAction({ ...action, stream: "" }),
    /invalid action/,
  );
  assert.throws(
    () => checkedAction({ ...action, inputs: [{ sequence: 0, x: 0, z: 0 }] }),
    /invalid action/,
  );
  assert.throws(
    () => checkedAction({ ...action, inputs: [{ sequence: 1, x: 2, z: 0 }] }),
    /invalid action/,
  );
  assert.throws(
    () => checkedAction({ ...action, extra: true }),
...
test("session rejects duplicate system write authority", () => {
  ...
  assert.throws(
    () =>
      new GameSession({
        port: new TestPort(),
        pack: pack(new TestPort(), undefined, undefined, [first, second]),
      }),
...
test("authored entity references use native membership without snapshot capture", () => {
  ...
  assert.throws(
    () => value.command("setLink", { target: "missing" }),
...
test("pending impacts survive save and a failed consumer step", () => {
  ...
  const restoredSystem: SystemDefinition = {
    ...system,
    run: (context) =>
      restoredObserved.push(context.impacts.map((event) => event.id)),
  };
...
test("native advance failure preserves the queued physical impact for retry", () => {
  ...
  const system: SystemDefinition = {
    ...
    writes: [],
    run: (context) =>
      observed.push(context.impacts.map((event) => event.sequence)),
  };
  const port = new TestPort();
  port.impacts = [impact];
...
test("a live consumer compacts a sustained impact stream", () => {
  ...
  const system: SystemDefinition = {
    ...
    writes: [],
    run: (context) =>
      seen.push(...context.impacts.map((event) => event.sequence)),
  };
  const port = new TestPort();
  const { value } = session(port, system);
  for (let sequence = 1; sequence <= 1100; sequence++) {
    port.impacts = [
      { ...impact, id: `impact.${sequence}`, sequence, time: sequence * 0.1 },
    ];
    value.step(0.1);
  }
  value.step(0.1);
...
test("an unconsumed impact backlog rejects the whole step at its bound", () => {
  ...
  port.impacts = [
    { ...impact, id: "impact.1025", sequence: 1025, time: 102.5 },
  ];
  ...
  const systems: SystemDefinition[] = [
    {
      ...
      writes: [],
      run: (context) =>
        first.push(...context.impacts.map((event) => event.sequence)),
    },
    {
      id: "test.slow-impact",
      version: 1,
      ...
      writes: [],
      run: (context) =>
        second.push(...context.impacts.map((event) => event.sequence)),
    },
  ];
  const port = new TestPort();
  const value = new GameSession({
  ...
  const cases = [
    ...
    { ...before, pendingActions: [{ kind: "bogus" } as never] },
    {
      ...
      kernel: {
        ...before.kernel,
        records: [
          { ...before.kernel.records[0], bytes: new Uint8Array([123]) },
        ],
      },
    },
  ];
      ...
      designate: command({
        ...
        run: () => ({
          ...
          creates: [
            {
              id: entity("order.1"),
              components: { "test.morale": { value: 1 } },
            },
          ],
        }),
      }),
            ...
            writes: [
              {
                entity: row.id,
                component: morale.id,
                value: { value: row.get(morale).value + 1 },
              },
            ],
          };
        },
