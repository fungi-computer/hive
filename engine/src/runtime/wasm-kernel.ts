import type { ActionRequest, ActionResult, ComponentDefinition, EntityId, KernelPort, KernelSnapshot, QueryRow, QuerySpec, RenderFact, WriteIntent } from "../contracts";

export interface WasmKernelBinding { load(json: string): void; query(json: string): string; advance(json: string): string; snapshot(): string; restore(json: string): void; render_facts(): string }
...
export function wasmKernelPort(binding: WasmKernelBinding): KernelPort {
  return {
    load(definition) { binding.load(new TextDecoder().decode(definition)); },
    query(spec: QuerySpec): readonly QueryRow[] {
      const ids = spec.components.map(component => component.id);
      return (JSON.parse(binding.query(JSON.stringify(ids))) as QueryWire[]).map(row => ({ id: row.id, get<V extends object>(definition: ComponentDefinition<V>): V {
        const value = row.components[definition.id];
        if (!value || typeof value !== "object") throw new Error(`query row ${row.id} lacks ${definition.id}`);
        return value as V;
      } }));
    },
