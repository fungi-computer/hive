...
const Own = component<{ value: number }>("test.own", { version: 1, fields: { value: "number" } });
const Other = component<{ value: number }>("test.other", { version: 1, fields: { value: "number" } });
const id = entity("actor:1");

export function authoringContractProof(): void {
  if (!Own.validate({ value: 2 }) || Own.validate({ value: "2" })) throw new Error("component validation failed");
  const guarded = system({ id: "test.rule", version: 1, writes: [Own], run(ctx) {
    ctx.write(Own, id, { value: 3 });
    let rejected = false;
    try { ctx.write(Other, id, { value: 1 }); } catch { rejected = true; }
