...
test("all work kinds retain saved claims before joint assignment", () => {
  ...
  assert.equal(allocateWork([{ task: dig, actor: null }],
    [{ worker: carrier, task: dig, cost: 0 }], candidate => candidate.cost,
    candidate => candidate.cost, rows => rows).length, 1);
});
...
test("conflicting task custody fails before the matcher runs", () => {
  ...
  assert.throws(() => allocateWork([{ task: a, actor }, { task: b, actor }], [], () => 0, () => 0, unexpected), /competing/);
  assert.throws(() => allocateWork([{ task: a, actor: null }, { task: a, actor: null }], [], () => 0, () => 0, unexpected), /duplicate/);
  assert.throws(() => allocateWork([], [{ worker: actor, task: a, cost: 1 }], () => 1, () => 1, unexpected), /unknown/);
  assert.deepEqual(allocateWork([{ task: a, actor }], [{ worker: actor, task: a, cost: 1 }], () => 1, () => 1, unexpected), []);
});
...
test("claimed work skips costing and an unreachable proposal is removed", () => {
  ...
  const result=allocateWork([{task:held,actor:busy},{task:open,actor:null}],
    ...
    candidates=>{ matchCalls++; return candidates; });
...
test("joint assignment refines only candidates that can still win", () => {
  ...
  const exact = new Map([[first, 100], [winner, 2], [distant, 60]]);
  const resolved: EntityId[] = [];
  const result = allocateWork(
    [{ task: first, actor: null }, { task: winner, actor: null }, { task: distant, actor: null }],
    [
    ...
    candidates => [candidates.reduce((best, candidate) => candidate.cost < best.cost ? candidate : best)],
  );
