...
type ProcessState = { version: number; definition: string; definitionVersion: number; station: EntityId; worker: EntityId | null; stageIndex: number; progressSeconds: number; enteredTick: number; phase: "waiting" | "working" | "complete" | "blocked"; blockedReason: string };
const state = (stageIndex = 0, phase: ProcessState["phase"] = "waiting"): ProcessState => ({ version: 2, definition: "ale", definitionVersion: 1, station: entity("station"), worker: phase === "working" ? entity("worker") : null, stageIndex, progressSeconds: 0, enteredTick: 0, phase, blockedReason: phase === "blocked" ? "blocked" : "" });
...
test("elapsed and blocked stages do not create attendance", () => { assert.equal(processAttendanceProvider(context(state(0, "blocked"), req("attended"), grain).ctx, [entity("worker")], new Set()).candidates.length, 0); assert.equal(processAttendanceProvider(context(state(), req("elapsed"), grain).ctx, [entity("worker")], new Set()).candidates.length, 0); });
