...
function context(process: ProcessState, requirements: ProcessRequirements, lots: readonly { readonly id: EntityId; readonly kind: string; readonly quantity: number; readonly container: EntityId }[], options: { readonly party?: EntityId; readonly workerParty?: EntityId; readonly attempts?: readonly WorkAttempt[]; readonly processCount?: number; readonly workerCount?: number; readonly tick?: number } = {}) {
  ...
  const value = (id: EntityId, definition: { readonly id: string }): object | undefined => {
    ...
    if (definition.id === OwnedByParty.id && (id === entity("station") || processIds.includes(id))) return { party };
  ...
  const ctx: WriteContext = {
    ...
    query<T extends object>(spec: QuerySpec<T>): readonly QueryRow<T>[] {
      ...
      const ids = componentIds.has(StagedProcess.id)
        ? processIds
        : componentIds.has(OwnedByParty.id)
          ? [...processIds, entity("station")]
          : componentIds.has(Position.id)
            ? [...workerIds, entity("station")]
            : workerIds;
...
test("blocked attendance releases its worker claim after acknowledgement", () => { const workers = [entity("worker"), entity("worker-1")]; const { ctx } = context(state(), req("attended"), grain, { processCount: 2, workerCount: 2, attempts: [blockedAttendance()] }); const provider = processAttendanceProvider(ctx, workers, new Set()); assert.equal(provider.claims.find(claim => claim.task === entity("process-0"))?.actor, null); assert.deepEqual(provider.candidates.map(candidate => candidate.task), [entity("process-1")]); });
