...
test("rehauled stock only moves to a strictly better cell, with capacity and claim bounds", () => {
  const zone = entity("zone");
  const low = stockpileCellRecords([{ zone, cell: [0, 3, 0], priority: 1, filterProfile: "materials", capacity: 3 }])[0];
  const high = stockpileCellRecords([{ zone, cell: [1, 3, 0], priority: 2, filterProfile: "materials", capacity: 3 }])[0];
  const equal = stockpileCellRecords([{ zone, cell: [2, 3, 0], priority: 1, filterProfile: "materials", capacity: 3 }])[0];
...
test("profile deny and malformed profile leave physical lots untouched", () => {
  const zone = entity("zone");
  const record = stockpileCellRecords([{ zone, cell: [0, 3, 0], priority: 2, filterProfile: "food", capacity: 3 }])[0];
