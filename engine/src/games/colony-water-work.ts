...
export function waterSupplyProvider(ctx: WriteContext, suspended: ReadonlySet<EntityId>): PreparedWorkProvider<Candidate> {
  ...
  for (const row of nativeRows) { if (attempts.has(row.id)) continue; const state = row.get(WaterSupplyWork), party = owners.get(row.id), order = row.get(WaterSupplyOrder); if (state.phase !== "queued" || order.revision !== state.request || !party || order.party !== party) continue; for (const worker of workers) { const vessel = pails.get(worker.id); if (!vessel || members.get(worker.id) !== party) continue; const options = contacts.map(contact => ({ cell: contact.at, approaches: contact.approaches })).filter(option => option.approaches.length); if (options.length) candidates.push({ worker: worker.id, task: row.id, vessel, options }); } }
