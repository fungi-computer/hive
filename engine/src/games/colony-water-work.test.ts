...
test("water provider sends at most sixteen authoritative centers", () => {
  ...
  const demand = row("demand", new Map([[WaterSupplyOrder, { revision: 1, process: null }], [WaterSupplyWork, { request: 1, attempt: 0, phase: "queued", actor: null, vessel: null, x: 0, y: 0, z: 0, approachX: 0, approachY: 0, approachZ: 0, reason: "" }]]));
...
test("completed demand is cleaned on the next phase without contact or material queries", () => {
  const demand = row("done", new Map([[WaterSupplyOrder, { revision: 1, process: null }], [WaterSupplyWork, { request: 1, attempt: 1, phase: "complete", actor: null, vessel: null, x: 0, y: 0, z: 0, approachX: 0, approachY: 0, approachZ: 0, reason: "" }]]));
...
test("queued planning stays bounded with many demands and keeps an active bound actor visible", () => {
  ...
  const demands = Array.from({ length: 256 }, (_, index) => row(`demand-${index}`, new Map([[WaterSupplyOrder, { revision: index + 1, process: null }], [WaterSupplyWork, { request: index + 1, attempt: 0, phase: "queued", actor: null, vessel: null, x: 0, y: 0, z: 0, approachX: 0, approachY: 0, approachZ: 0, reason: "" }]])));
  demands.push(row("active", new Map([[WaterSupplyOrder, { revision: 257, process: null }], [WaterSupplyWork, { request: 257, attempt: 1, phase: "approaching", actor: id("worker-z"), vessel: id("pail-z"), x: 0, y: 1, z: 0, approachX: 0, approachY: 1, approachZ: 0, reason: "" }]])));
