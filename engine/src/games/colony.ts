...
export const colonyPack: GamePack = {
  ...
  presentation: {
    ...
    controls: [
      ...
      ...(["timber-floor", "timber-wall", "timber-roof", "timber-bed"] as const).map(catalog => ({ id: catalog, label: `Build ${catalog.replace("timber-", "")}`, command: "build", input: { catalog, ...(catalog === "timber-floor" || catalog === "timber-roof" || catalog === "timber-bed" ? { orientation: "north" } : {}) }, target: "world-surface" as const, designation: catalog === "timber-wall" ? ["point", "line", "rectangle"] as const : catalog === "timber-bed" ? ["point"] as const : ["point", "rectangle"] as const })),
