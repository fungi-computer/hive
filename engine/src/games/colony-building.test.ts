...
test("building expands deterministic point, line and rectangle designations without workers", () => {
  ...
  const point = colonyBuildCommand.run(context, { catalog: "timber-floor", orientation: "north", target: { cell: [0, 17, 0] } });
  assert.equal(point.actions.length, 1);
  assert.equal(point.actions[0].kind, "plan-construction");
  const line = colonyBuildCommand.run(context, { catalog: "timber-wall", target: { area: { start: [0, 17, 0], end: [2, 17, 0] } } });
  ...
  const rectangle = colonyBuildCommand.run(context, { catalog: "timber-floor", orientation: "north", target: { area: { start: [0, 17, 0], end: [1, 17, 1] } } });
...
test("building skips an already planned site deterministically and rejects an invalid later cell", () => {
  const duplicate = colonyBuildCommand.run({
    query: () => [{ id: entity("colony.build.timber-wall.0.18.0.north") }],
    physicalContacts: cells => cells.map(() => ({ solid: true, sealedTop: false, outside: false })),
  }, { catalog: "timber-wall", orientation: "north", target: { cell: [0, 17, 0] } });
  assert.deepEqual(duplicate.actions, []);
  let calls = 0;
  assert.throws(() => colonyBuildCommand.run({ query: () => [], physicalContacts: cells => {
    calls += 1;
