...
test("Colony cancelDig submits native cancellation for the exact active worker", () => {
  ...
  const attempt: WorkAttempt = { continuationOwner: "external", key: { task: order.id, generation: 1 }, worker, execution: { pool: entity("host"), initiatingPlayer: null, policyId: "excavation" }, phase: { kind: "executing", operation: { attempt: { task: order.id, generation: 1 }, sequence: 1 }, activity: { kind: "excavation", cell: [0, 12, 0], expectedMaterial: 1, replacementMaterial: 0 } } };
