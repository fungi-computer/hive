...
import { Destination, Position, Selected, encodeDefinition, move } from "../sdk/common";
...
export const formations = system({ id: "formations.retreat", version: 1, reads: [FormationMember, Morale, Position], run(ctx) {
  for (const unit of ctx.query(query(FormationMember, Morale, Position))) {
    ...
    if (morale.value < morale.retreatBelow) ctx.action(move(unit.id, { x: -4, y: 0, z: -4 }, 0));
  }
});
...
export const formationsPack: GamePack = { id: "formations", version: 1, components: [Position, Destination, Selected, FormationMember, Morale], systems: [formations], definition: encodeDefinition("formations", [Position, Destination, Selected, FormationMember, Morale], formationInitial) };
