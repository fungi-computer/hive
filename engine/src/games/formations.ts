...
export const formations = system({ id: "formations.retreat", version: 1, reads: [FormationMember, Morale, FormationOrder], writes: [Destination], run(ctx) {
  for (const unit of ctx.query(query<{ group: EntityId; slot: number; value: number; retreatBelow: number }>(FormationMember, Morale, FormationOrder))) {
    if (unit.value.value < unit.value.retreatBelow) ctx.action(groupOrder(unit.value.group, { x: -4, y: -4, z: 0 }, 0));
  }
});
const groupId = entity("formations.group.1");
const formationInitial = [{ id: groupId, components: {} }, ...[1, 2, 3].map(slot => ({ id: entity(`formations.unit.${slot}`), components: { "hive.position": { x: slot, y: 0, z: 0, facing: 0 }, "formations.member": { group: groupId, slot }, "formations.morale": { value: 80, retreatBelow: 25 } } }))];
