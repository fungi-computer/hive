...
test("upper placement includes the supporting top itself and neighboring cells", () => {
  assert.deepEqual(structureAnchor(frame, [0, 4, 0]), [0, 4, 0]);
  assert.deepEqual(upperPlacementCandidates(frame, [0, 4, 0]), [[0, 4, 0], [-1, 4, 0], [1, 4, 0], [0, 4, -1], [0, 4, 1]]);
  assert.deepEqual(upperPlacementCandidates(frame, [8, 4, 8]), []);
});
test("pointer selection returns only a candidate plane cell", () => {
  assert.deepEqual(upperPlacementAt(project(0, 4.5, 0), frame, [0, 4, 0]), [0, 4, 0]);
