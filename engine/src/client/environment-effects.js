...
export function drawEnvironmentEffects(graphics, visuals, { project, camera, now, maxY = Infinity }) {
  ...
  for (const visual of visuals.slice(0, 64)) {
    ...
    if (visual.kind === "fire") {
      ...
      graphics.rect(x - 5 * scale, y - (5 + phase) * scale, 10 * scale, (5 + phase) * scale).fill({ color: 0xc96035, alpha: visual.intensity });
      graphics.rect(x - 2 * scale, y - (7 - phase) * scale, 4 * scale, (7 - phase) * scale).fill({ color: 0xf1c16f, alpha: visual.intensity });
      ...
      graphics.ellipse(x + offset, y - 4 * scale, 15 * scale, 7 * scale).fill({ color: 0x77786d, alpha: visual.intensity * 0.28 });
      graphics.ellipse(x - 5 * scale - offset, y - 10 * scale, 10 * scale, 6 * scale).fill({ color: 0x9b9d8c, alpha: visual.intensity * 0.20 });
    }
  }
}
