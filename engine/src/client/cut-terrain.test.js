...
test("consecutive batches preserve IDs across actors, rails, state/texture changes and 16-bit splits",()=>{
  ...
  const replaced = owner.update(newGeometry)[0].display;
  assert.notEqual(replaced, mesh, "changed geometry is prepared in a detached mesh");
  const changedPositions = replaced.geometry.positions;
  assert(Math.abs(changedPositions[0] - small[0].projected[0].x - 1) < 0.001);
  assert.equal(owner.update(newGeometry)[0].display, replaced, "unchanged geometry keeps its mesh");
  const restored = owner.update(small)[0].display;
  assert.notStrictEqual(restored.geometry.positions, changedPositions);
  assert(Math.abs(restored.geometry.positions[0] - small[0].projected[0].x) < 0.001);
