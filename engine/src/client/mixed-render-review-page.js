...
for (const orientation of MIXED_FIXTURE_ORIENTATIONS) camera.add(new Option(orientation, orientation));
for (const orientation of MIXED_FIXTURE_ORIENTATIONS) object.add(new Option(orientation, orientation));

const app = new Application();
await app.init({
  resizeTo: host,
  backgroundColor: 0x87928d,
  antialias: false,
  resolution: 1,
  preference: "webgl",
...
function applyFocus() {
  ...
  for (const record of owner.compiled.records) {
    ...
    if (record.role === "actor") {
      visible =
        focus.value === "all" ||
        (focus.value === "scene" &&
          ["fixture:goblin", "fixture:goblin:stair-middle", "fixture:goblin:bed-side-left"].includes(record.id)) ||
  ...
  status.textContent = `${owner.compiled.records.length} original-art records · spatial geometry order · ${focus.value} focus`;
}

function render() {
  owner?.dispose();
  owner = createMixedRenderReviewOwner({
    art: staticPack.art,
    terrainPack,
    cameraOrientation: camera.value,
    objectOrientation: object.value,
  });
  const bounds = owner.compiled.records.reduce(
    (box, record) => ({
      left: Math.min(box.left, record.screenBounds.left),
      right: Math.max(box.right, record.screenBounds.right),
      top: Math.min(box.top, record.screenBounds.top),
      bottom: Math.max(box.bottom, record.screenBounds.bottom),
    }),
  ...
  owner.container.position.set(
    (app.screen.width - width * scale) / 2 - bounds.left * scale,
