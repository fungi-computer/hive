...
const cell = (x, volume = 1) => ({ at: [x, 2, 0], liquidVolumeM3: volume, level: 3 });
...
test("water reconciliation retains unchanged keyed sprites and disposes removed cells", () => {
  ...
  const options = {
    key: waterCellKey,
    create: value => { const sprite = { id: value.at[0] }; created.push(sprite); return sprite; },
  ...
  assert.strictEqual(second.get(waterCellKey(cell(2))).sprite, first.get(waterCellKey(cell(2))).sprite);
...
test("water reconciliation removes zero-volume cells", () => {
  const disposed = [];
  const options = { key: waterCellKey, create: () => ({}), update() {}, dispose: sprite => disposed.push(sprite) };
