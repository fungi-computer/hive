...
export function createWorldDepthLayer({ width, height, resolution = 1, roleOrder } = {}) {
  ...
  function update(items, towardCamera) {
    ...
    for (const [index, item] of ordered.entries()) {
      ...
      const uniforms = record.shader.resources.depthUniforms.uniforms;
      uniforms.uOriginDepth = originDepth(item, bounds.basis);
      uniforms.uLocalMin = finite(range.min, "depth min"); uniforms.uLocalMax = finite(range.max, "depth max");
      uniforms.uNearDepth = bounds.nearDepth; uniforms.uFarDepth = bounds.farDepth;
  ...
  return Object.freeze({ container, target, texture: renderTexture, picker, update,
    ...
    renderTransparent(renderer, items = []) {
      ...
      for (const [index, item] of ordered.entries()) {
        ...
        record.shader.resources.uDepthTexture = item.depthTexture.source;
        const uniforms = record.shader.resources.depthUniforms.uniforms;
        uniforms.uOriginDepth = originDepth(item, activeBounds.basis);
        uniforms.uLocalMin = finite(item.depthFrame.depthRange.min, "depth min");
        uniforms.uLocalMax = finite(item.depthFrame.depthRange.max, "depth max");
        uniforms.uNearDepth = activeBounds.nearDepth;
        uniforms.uFarDepth = activeBounds.farDepth;
        uniforms.uAlpha = item.alpha;
