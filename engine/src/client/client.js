...
export function createHiveClient({ root, mode, title, subtitle, source, runtime = null, projection = (x, y, z, dimensions = { x: 36, y: 24 }) => ({ x: x * dimensions.x + z * dimensions.x * 0.5, y: z * dimensions.y - y * dimensions.y }), unprojection = (x, y, dimensions = { x: 36, y: 24 }) => ({ x: x / dimensions.x - y / dimensions.y * 0.5, y: 0, z: y / dimensions.y })) {
  ...
  const camera = { x: 0, y: 0, zoom: canvasHost.clientWidth >= 900 ? 2 : 1, pan(dx, dy) { this.x -= dx; this.y -= dy; draw(); }, zoomBy(delta, point = { x: app.screen.width / 2, y: app.screen.height / 2 }) { const before = { x: (point.x - this.x) / this.zoom, y: (point.y - this.y) / this.zoom }; this.zoom = Math.max(1, Math.min(4, this.zoom + delta)); this.x = point.x - before.x * this.zoom; this.y = point.y - before.y * this.zoom; draw(); }, reset() { this.x = this.y = 0; this.zoom = canvasHost.clientWidth >= 900 ? 2 : 1; draw(); } };
