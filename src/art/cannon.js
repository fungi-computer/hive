...
function wheel(parent, z) {
  const result = cylinder(parent, "#694733", -0.36, 0.4, z, 0.38, 0.42, 0.16, 10);
  result.rotation.x = Math.PI / 2;
  cylinder(parent, "#b27d48", -0.36, 0.4, z + (z < 0 ? -0.086 : 0.086), 0.2, 0.2, 0.018, 10).rotation.x = Math.PI / 2;
}

function carriage(parent) {
  box(parent, "#8e6a43", -0.35, 0.22, 0, 1.35, 0.34, 0.62);
  box(parent, "#ba925e", -0.35, 0.43, 0, 1.08, 0.14, 0.5);
  const axle = cylinder(parent, "#5d3e2b", -0.36, 0.4, 0, 0.08, 0.08, 1.05, 8);
...
function barrel(parent) {
  ...
  const muzzle = cylinder(parent, "#252c2a", 1.35, 0.77, 0, 0.25, 0.25, 0.16, 8);
