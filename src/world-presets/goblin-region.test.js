...
test("player and host grants differ; paused/no-change actions produce no false events", () => {
  ...
  assert(
    !p.authorize(
      "goblin-player",
      { ...dig, command: { ...dig.command, party: "other" } },
      s,
    ),
  );
  assert.deepEqual(p.execute(s, { kind: "advance", ticks: 120 }), {
    status: "applied",
    result: { tick: 0, advanced: 0 },
    events: [],
  });
  assert.deepEqual(
    p.execute(s, { kind: "set-paused", paused: true }).events,
    [],
  );
...
test("paused admitted dig becomes real pawn work; reconstruction preserves running work and one soil result", () => {
  ...
  assert.deepEqual(
    admitted.result.createdJobs,
    s.clearing.jobs.map((j) => j.id),
  );
  assert.equal(s.clearing.tick, 0);
  assert.deepEqual(s.clearing.terrain.exports, []);
  p.execute(s, { kind: "set-paused", paused: false });
  p.execute(s, { kind: "advance", ticks: 10 });
  const resumed = p.parseState(JSON.parse(JSON.stringify(s)));
  assert.equal(resumed.clearing.paused, false);
  assert.equal(resumed.clearing.tick, 10);
  assert.deepEqual(resumed, s);
  p.execute(s, { kind: "advance", ticks: 50 });
  p.execute(resumed, { kind: "advance", ticks: 50 });
  assert.deepEqual(resumed, s);
  assert.equal(s.clearing.terrain.exports.length, 1);
  assert.equal(
    s.clearing.materials.lots
...
test("maintained optimizer identity is immutable and mismatched durable program refuses reopen without mutation", async (t) => {
  ...
  assert(Object.isFrozen(colony));
  assert(p.id.includes(colony.buildId));
  assert(p.id.length <= 160);
  assert.throws(
    () => createGoblinRegionProgram({ ...colony, buildId: "invented" }),
    /invalid-optimizer-build-identity/,
  );
  ...
  assert.throws(
    () =>
      openRegion({
        owner,
        region: "goblin-law",
        program: createGoblinRegionProgram({
          ...colony,
          buildId: `${"0".repeat(64)}:${"1".repeat(64)}`,
        }),
      }),
    /region-identity-conflict/,
  );
