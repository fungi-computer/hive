...
test('visible wet clearing reads edited voxels and actual finite water, surviving a fresh owner', () => {
  ...
  const flowing = adapter.advance(cut, 60).state;
  ...
  assert.ok(water.depthM > 0);
  assert.equal(water.depthM * 1000, water.massKg);
  assert.equal(water.baseYM, target[1] * scene.spacingM[1]);
  assert.ok(Math.abs(water.rimYM - water.baseYM - scene.spacingM[1]) < 2e-15);
