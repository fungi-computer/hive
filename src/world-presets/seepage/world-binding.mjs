...
export function restoreWorld(identity, checkpoint) {
  ...
  // fields, invalid legacy/current envelopes and capacity violations.
...
export function worldSnapshot(world, legacy = false) {
  ...
  // The retained combined v1 owner admitted a recursively ordered recipe and
  // saved that exact identity ordering. Preserve that format while validating;
  // only the explicit v2 migration adopts the current world's encoding.
  return { schema: 2, identity: ordered(saved.identity), revision: saved.revision, changes: saved.changes };
}
