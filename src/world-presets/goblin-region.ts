import { z } from "zod";
import { optimizerBuildIdentity, type Optimizer } from "../engine/colony/loader.ts";
import { createClearing, step } from "../clearing.ts";
import { admitCommand } from "../orders.ts";
import { commandSchema } from "../command-schema.ts";
import { parseSerializedClearing, parseLiveClearing, serializeClearing, type SerializedClearing } from "../clearing-state.ts";
import type { RegionProgram, RegionTransition } from "../engine/region/index.ts";
const stateSchema = z.object({ clearing: z.unknown().transform(parseSerializedClearing) }).strict();
const inputSchema = z.discriminatedUnion("kind", [
  ...
  z.object({ kind: z.literal("advance"), ticks: z.number().int().min(1).max(120) }).strict(),
]);
  ...
  return {
    ...
    authorize(principal, input) {
      return input.kind === "advance"
        ...
        : principal === "goblin-player" && (input.kind === "set-paused" || input.command.party === "home");
    },
    execute(candidate, input): RegionTransition {
      ...
      if (input.kind === "order") {
        ...
        if (result.status === "rejected") return { status: "rejected", result: { reason: result.reason } };
        candidate.clearing = serializeClearing(clearing);
        return { status: "applied", result: { createdJobs: result.createdJobs, tick: clearing.tick }, events: [{ kind: "order-admitted", createdJobs: result.createdJobs }] };
      }
      if (input.kind === "set-paused") {
        const changed = clearing.paused !== input.paused;
        clearing.paused = input.paused;
        candidate.clearing = serializeClearing(clearing);
        return { status: "applied", result: { paused: clearing.paused }, events: changed ? [{ kind: "pause-changed", paused: clearing.paused }] : [] };
      }
      const before = clearing.tick;
      for (let count = 0; !clearing.paused && count < input.ticks; count++) step(clearing, optimizer);
      candidate.clearing = serializeClearing(clearing);
      return { status: "applied", result: { tick: clearing.tick, advanced: clearing.tick - before }, events: clearing.tick !== before ? [{ kind: "world-advanced", from: before, to: clearing.tick }] : [] };
    },
  };
}
