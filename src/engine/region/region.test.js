...
function fixture(t, limits, program = createQuarryRegionProgram) {
  ...
  const owner = sqliteTestOwner(db, (statement) => {
    ...
    if (failRecord && statement.includes("hive_region_records") && statement.startsWith("INSERT"))
...
test("record pages cap returned bytes and continue by key", (t) => {
  const f = fixture(t, {}, () => {
    const base = createQuarryRegionProgram();
    return { ...base, id: "quarry-record-page-v1", initial: () => ({ state: base.initial().state, records: Array.from({ length: 5 }, (_, index) => ({ key: `page/${index}`, bytes: new Uint8Array(300_000) })) }) };
  });
  const region = f.open();
