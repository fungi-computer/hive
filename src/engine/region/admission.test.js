...
test('unconditional intent follows advancing world and replays once while conditional revision remains enforced',t=>{
 ...
 for(let i=0;i<4;i++)r.dispatch('player',{replayEpoch:0,id:`tick-${i}`,command:{kind:'tick'}});
 const take={replayEpoch:0,id:'take',command:{kind:'take'}};
 const receipt=r.dispatch('player',take);assert.equal(receipt.status,'applied');assert.equal(receipt.revision,5);
 r.dispatch('player',{replayEpoch:0,id:'tick-next',command:{kind:'tick'}});
 ...
 const stale=r.dispatch('player',{replayEpoch:0,id:'conditional',expectedRevision:0,command:{kind:'take'}});
...
test('unconditional intent still rolls back and checks authority',t=>{
 const f=setup(t),r=f.open(),before=r.readCommitted(),take={replayEpoch:0,id:'take',command:{kind:'take'}};
