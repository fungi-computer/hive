...
import { originalBuilderSchema } from "./original-pack.ts";
...
const geometrySchema = z.discriminatedUnion("kind", [
  ...
  z.strictObject({
    ...
    pack: z.literal("hive-brewhouse-v1"),
    asset: originalBuilderSchema,
  }),
  z.strictObject({ kind: z.literal("box"), size: vector(0.01, 8) }),
  z
    .strictObject({
      kind: z.literal("cylinder"),
      radiusTop: z.number().min(0).max(4),
      radiusBottom: z.number().min(0).max(4),
      height: z.number().min(0.01).max(8),
      segments: z.number().int().min(3).max(64),
    })
    .refine(
      (g) => g.radiusTop + g.radiusBottom > 0,
      "Cylinder needs a nonzero radius",
    ),
]);
export const sceneNodeSchema = z
  ...
  .refine(
    (node) => node.geometry.kind !== "group" || node.material === null,
    "Group materials do not inherit; edit a geometry node",
  );
export const sceneDocumentSchema = z
  .strictObject({
    ...
    nodes: z.array(sceneNodeSchema).max(SCENE_LIMITS.nodes),
  })
  .superRefine(({ nodes }, context) => {
    const byId = new Map(nodes.map((node) => [node.id, node]));
    const issue = (message: string) =>
      context.addIssue({ code: "custom", path: ["nodes"], message });
    ...
    for (const node of nodes) {
      ...
      while (parentId !== null) {
        if (visited.has(parentId)) {
          issue("Hierarchy must be acyclic");
          break;
        }
        visited.add(parentId);
        if (visited.size > SCENE_LIMITS.depth) {
          issue("Hierarchy depth exceeds limit");
          break;
        }
        const parent = byId.get(parentId);
        if (!parent || parent.geometry.kind !== "group") {
          issue("Parent must reference an existing group");
          break;
        }
