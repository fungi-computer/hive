...
const output = process.argv[3] ?? ".botanical/water-inspection-input/browser";
assert.equal(
...
const evidence = {
  base,
  fixture: "initial startup, earned [0,14,128] dig, exact hollow inspection and paused interaction guards",
  runtimePin: "5bb8b7c0800b4cb620ccd4b82b754d5fbae6ac1a",
  errors: [],
  consoleErrors: [],
  failedRequests: [],
  moduleResponses: [],
...
try {
  ...
  const hollow = await page.evaluate((height) => window.__GOBLIN.project(6.65, 8.65, height, 0), floorHeight);
  const card = page.getByRole("region", {name:"Field water details", exact:true});
  async function inspect(name) {
    await page.mouse.click(hollow.x, hollow.y);
    await card.waitFor({state:"visible", timeout:10000});
    const observed = await page.evaluate(() => ({state:window.__GOBLIN.state, selection:window.__GOBLIN.selection}));
    ...
    assert.deepEqual(selected.reference, {binding:FIELD_WATER.id, nodeId:"reservoir:column-p0-p128"});
    const node = terrainFacts(observed.state.terrain).soil.nodes.find(node => node.nodeId === selected.reference.nodeId);
    assert(node && node.kind === "pit");
    const fact = observed.selection.fieldWater;
    assert.equal(fact.x, 7);
    assert.equal(fact.z, 9);
    assert.equal(fact.litres, node.massKg / FIELD_WATER.kgPerUnit * FIELD_WATER.litresPerUnit);
    assert.equal(fact.wholeMeasures, Math.floor(node.massKg / FIELD_WATER.kgPerUnit));
    const text = fieldInspectionText(fact);
    assert.equal(await card.locator('[data-field-water="litres"]').textContent(), text.stock);
    assert.equal(await card.locator('[data-field-water="measures"]').textContent(), text.measures);
    assert((await card.textContent()).includes(text.access));
    evidence.waterObservation = {litres:fact.litres, wholeMeasures:fact.wholeMeasures,
      observedStock:fact.litres===0 ? "dry" : fact.litres<1 ? "fractional" : "whole measures present",
      floorHeight, hollow, reference:selected.reference};
    await writeFile(join(output, `${name}.json`), JSON.stringify(observed,null,2));
    await page.screenshot({path:join(output, `${name}.png`)});
  }
  await page.keyboard.press("Escape");
  await page.waitForFunction(() => window.__GOBLIN.selection.tool === null);
  await inspect("field-inspected");
  evidence.checks.push("earned hollow click selects exact node and displays real stock/whole measures without advancing time");
  await card.getByRole("button", {name:"Close field water details", exact:true}).click();
  await card.waitFor({state:"hidden", timeout:10000});
  assert.equal((await page.evaluate(() => window.__GOBLIN.selection)).inspectedTarget, null);
  ...
  assert.notEqual((await page.evaluate(() => window.__GOBLIN.selection)).inspectedTarget?.kind, "field-water");
  await card.waitFor({state:"hidden", timeout:10000});
  assert.equal(await card.count(), 0);
  evidence.checks.push("close removes the card; moved box gesture does not become field inspection");

  await page.getByRole("button", {name:"Build",exact:true}).click();
  await page.locator("#dig-tool").click();
  const adjacent = await page.evaluate(() => window.__GOBLIN.project(8,9,0,0));
  await page.mouse.click(adjacent.x,adjacent.y);
  await page.waitForFunction(() => window.__GOBLIN.state.jobs.some(job => job.kind === "dig" && job.voxel[0]===1 && job.voxel[1]===14 && job.voxel[2]===128));
  const toolState = await page.evaluate(() => ({state:window.__GOBLIN.state,selection:window.__GOBLIN.selection}));
  ...
  await writeFile(join(output,"paused-tool-admission.json"),JSON.stringify(toolState,null,2));
  await page.mouse.click(adjacent.x,adjacent.y,{button:"right"});
  await page.waitForFunction(() => window.__GOBLIN.selection.tool === null);
  await card.waitFor({state:"hidden", timeout:10000});
  assert.equal(await card.count(),0);
  await inspect("field-reinspected");
  evidence.checks.push("armed Dig keeps its actual paused order path; rightclick disarms; exact hollow can be inspected again without another physical cut");
