// One built release candidate, one preview, existing bounded physical input fixture.
...
const root = process.cwd(),
  out = ".botanical/gameplay-water-release";
...
try {
  ...
  await run(["node_modules/vite/bin/vite.js", "build"], "window-build");
  const manifest = {};
  async function inventory(dir) {
    for (const entry of await readdir(dir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) await inventory(path);
        ...
        manifest[path.slice(5)] = createHash("sha256")
  ...
  await writeFile(
    `${out}/dist-sha256.json`,
    JSON.stringify({ runtimePin, files: manifest }, null, 2),
    { flag: "wx" },
  );
