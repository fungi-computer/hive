...
const output = process.argv[3] ?? ".botanical/gameplay-water-input-v3/browser";
assert.equal(
...
const evidence = {
  base,
  fixture:
    "public Sedge parking, earned [0,14,128] dig, exposed hollow inspection and paused interaction guards",
...
try {
  ...
  // Functional readiness is bounded separately from the unchanged 60s target.
  evidence.startup = {
    targetMs: 60000,
    functionalGuardMs: 90000,
    observedReadyMs: await page.evaluate(() => performance.now()),
  };
  evidence.startup.targetMet = evidence.startup.observedReadyMs <= 60000;
  await page.locator("#reset").click();
  await page.waitForFunction(
    () => window.__GOBLIN.state.paused && window.__GOBLIN.state.tick === 0,
  );
  // Move the known foreground actor through ordinary player controls. The
  // previous run clicked Sedge's visible hat over the physical hollow.
  const sedge = await page.evaluate(() =>
    window.__GOBLIN.project(10, 12, 0.6, 0),
  );
  await page.mouse.click(sedge.x, sedge.y);
  await page.locator("#recruit").click();
  await page.locator("#select-sedge").click();
  await page.waitForFunction(
    () =>
      window.__GOBLIN.selection.selectedIds.length === 1 &&
      window.__GOBLIN.selection.selectedIds[0] === "sedge",
  );
  await page.locator("#draft").click();
  await page.waitForFunction(() => window.__GOBLIN.state.actors.sedge.drafted);
  const parking = await page.evaluate(() =>
    window.__GOBLIN.project(11, 13, 0, 0),
  );
  await page.mouse.click(parking.x, parking.y, { button: "right" });
  await page.waitForFunction(
    () => window.__GOBLIN.state.actors.sedge.path.length > 0,
  );
  await page.locator("#pause").click();
  await page.waitForFunction(
    () => {
      const p = window.__GOBLIN.state.actors.sedge;
      return p.x === 11 && p.z === 13 && p.path.length === 0;
    },
    undefined,
    { timeout: 15000 },
  );
  ...
  const startTick = await page.evaluate(() => window.__GOBLIN.state.tick);
  await writeFile(
    join(output, "parked.json"),
    JSON.stringify(await page.evaluate(() => window.__GOBLIN.state), null, 2),
  );
  evidence.checks.push(
    "public Recruit/Draft/Go parks Sedge clear of the hollow without changing art or direct state",
  );
  ...
  assert.equal(admitted.tick, startTick);
  assert.deepEqual(
  ...
  if (page) {
    let timeout;
    try {
      evidence.failureSelection = await Promise.race([
        page.evaluate(() => ({
          selection: window.__GOBLIN?.selection,
        ...
        new Promise((_, reject) => {
          timeout = setTimeout(
            () => reject(new Error("bounded selection read expired")),
            2000,
          );
        }),
      ]);
