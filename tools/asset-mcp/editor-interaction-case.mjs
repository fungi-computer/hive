...
export async function interactionCase(page, frame, output, original) {
  ...
  const normalized = originalChildren.filter(
    (node) =>
      node.userData?.serializationNormalization ===
      "original-detached-light-target-v1",
  );
  assert(
    normalized.length > 0,
    "Original detached targets became explicit scene nodes",
  );
  for (const light of originalChildren.filter((node) =>
    ["DirectionalLight", "SpotLight"].includes(node.type),
  ))
    assert(
      originalChildren.some((node) => node.uuid === light.target),
      "Light target link resolves in native scene graph",
    );
