...
Cancellation assertion strengthened after browser cold-pan evidence: the canceled 128-region request must now produce **fewer than 128 patches and no completion**, in addition to no old-stream events after replacement begins. The earlier no-leakage-only assertion did not establish interruption. `--running true` runs the same contract against a fresh unpaused world, interleaves resume rather than pause, proves physical time advances, and pauses its own world during cleanup. Reconnect remains a direct socket protocol exercise.

The strengthened paused hosted proof passed against the September 20 v4 backend: only 5/128 canceled patches, no completion; `run-u2867.scope`, invocation `62039106fde34f6f948b89b2108d2888`, exit 0, evidence `.botanical/terrain-stream-cancel-paused-v4`. The unpaused equivalent also passed: `run-u2868.scope`, invocation `99f7513f395846b09dc3d79efeafc792`, exit 0, evidence `.botanical/terrain-stream-cancel-running-v4`. These prove the deployed socket can interrupt streams in both modes. They do not explain the browser's delayed replacement delivery or establish a cause involving `waitUntil`.
