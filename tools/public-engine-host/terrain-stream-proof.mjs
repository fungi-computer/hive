...
const running=args.get('--running')==='true';
const route=`${base}/v1/colony-performance-256-8`,token=randomBytes(32).toString('hex'),started=Date.now();
const report={endpoint:base,origin,running,startedAt:new Date().toISOString(),http:[],events:[],requests:[],checks:[],success:false};
...
try{
 ...
 await command(running?'resume':'pause');const patches=await complete(first,request);assert(patches.some(p=>p.patch.faces.length>0));report.checks.push(`incremental useful region patches precede completion; ${running?'resume':'pause'} interleaved`);
