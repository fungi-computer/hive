...
const report={endpoint:base,origin,startedAt:new Date().toISOString(),http:[],events:[],requests:[],checks:[],success:false};
...
function checked(event,request){
 ...
 if(event.kind==='patch'){assert(request.regions.some(key=>key.join(',')===event.patch.key.join(',')),'unrequested patch');assert(Array.isArray(event.patch.faces));assert(Array.isArray(event.patch.surfaces));for(const face of event.patch.faces)assert(face.cell[1]<=request.level);}
}
...
try{
 ...
 await http('observe');await command('pause');const initial=await http('observe');assert(initial.observation.paused);const first=await connect(),terrain=first.observation.observation.terrain;
 assert.equal(terrain.baseline.protocolVersion,4);const identity={epoch:first.observation.observation.epoch,terrainRevision:terrain.revision,level:terrain.baseline.bounds.maxY-1};assert(Number.isInteger(identity.terrainRevision));
 const regions=Array.from({length:32},(_,i)=>[i%8-4,Math.floor(i/8)-2]);const request={type:'terrain-regions',requestId:1,...identity,regions};first.send(request);
 ...
 await command('pause');const patches=await complete(first,request);assert(patches.some(p=>p.patch.faces.length>0));report.checks.push('incremental useful region patches precede completion; pause interleaved');
 const canceled={...request,requestId:2,regions:Array.from({length:128},(_,i)=>[i%16-8,Math.floor(i/16)-4])};first.send(canceled);
 await first.wait(e=>e.type==='terrain-regions'&&e.event.requestId===2&&e.event.kind==='patch');first.send({type:'terrain-cancel',requestId:2});const replacement={...request,requestId:3,regions:[[8,0],[8,1]]};first.send(replacement);await complete(first,replacement);
 ...
 const second=await connect();assert.equal(second.observation.observation.time,initial.observation.time);const resumed={...reconnect,regions:missing};second.send(resumed);const recovered=await complete(second,resumed);assert(recovered.every(e=>!seen.has(e.patch.key.join(','))));assert.equal(seen.size+recovered.length,reconnect.regions.length);report.reconnect={retained:seen.size,requestedMissing:missing.length};report.checks.push('reauthenticated same world; only missing region keys requested and returned');
 const final=await http('observe');assert(final.observation.paused);assert.equal(final.observation.time,initial.observation.time,'terrain reads advanced paused simulation time');report.pausedTime={before:initial.observation.time,after:final.observation.time};report.checks.push('terrain reads do not advance paused simulation time');report.success=true;
}catch(error){report.error=String(error).replaceAll(token,'[redacted]');process.exitCode=1;}finally{for(const socket of sockets)socket.close();report.finishedAt=new Date().toISOString();await save();console.log(JSON.stringify({success:report.success,checks:report.checks,error:report.error}));}
