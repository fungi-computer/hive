...
const optimizer = await loadOptimizer(
  await WebAssembly.compile(await readFile("src/engine/colony/colony.wasm")),
);
const digVoxel: [number, number, number] = [0, 14, 128];
function fixture(t: { after(fn: () => void): void }) {
  ...
  const reopen = () =>
    openRegion({
      owner,
      region: "delegated-goblin",
      program: createGoblinRegionProgram(optimizer),
    });
  const region = reopen();
    ...
    try {
      ...
      for (const part of parts) {
        ...
        if (part.result.isError)
          throw new Error(JSON.stringify(part.result.content));
...
function controller(
  region: ReturnType<typeof fixture>["region"],
  actor = "rowan",
) {
  return goblinController(region, { actor, knownDigVoxels: [digVoxel] });
}
...
test("personal rest admission stays paused; actor/knowledge grants are copied and observations narrow", async (t) => {
  ...
  const selected = {
    actor: "rowan",
    knownDigVoxels: [[...digVoxel] as [number, number, number]],
  };
  const module = goblinController(region, selected);
  selected.actor = "sedge";
  selected.knownDigVoxels[0][0] = 12;
  ...
  assert.deepEqual(
    after.state.clearing.materials,
    before.state.clearing.materials,
  );
  assert.deepEqual(
    after.state.clearing.jobs.map((job) => job.id),
    receipt.result.createdJobs,
  );
...
test("ungranted actor/cell and host-time inputs cannot mutate the real region", async (t) => {
  ...
  await assert.rejects(
    invoke(controller(region, "not-a-member"), "observe", {}),
  );
  assert.equal(
    db.prepare("SELECT COUNT(*) AS n FROM hive_region_receipts").get()!.n,
    0,
  );
});
...
test("paused personal dig has a durable receipt; reconstructed retry cannot duplicate pawn work or soil", async (t) => {
  ...
  assert.deepEqual(
    receipt.result.createdJobs,
    admitted.state.clearing.jobs.map((job) => job.id),
  );
  const resumed = reopen();
  assert.deepEqual(
    await invoke(controller(resumed), "command", input),
    receipt,
  );
  assert.deepEqual(resumed.readCommitted(), admitted);
  await assert.rejects(
    invoke(controller(resumed), "command", {
      ...input,
      command: { kind: "rest" },
    }),
  );
  ...
  assert.deepEqual(
    await invoke(controller(reopen()), "command", input),
    receipt,
  );
