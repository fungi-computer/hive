...
separate direction. Inspect and use its actual public package/components/styles.
Hive still owns world selection, designation, commands and the existing OpenTUI
keymap. The package currently declares React18 peers while Hive uses React19;
an isolated native compatibility probe is in progress, not a shipped adoption.
