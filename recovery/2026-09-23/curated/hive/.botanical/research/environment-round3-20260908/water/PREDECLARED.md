...
## Tiny ownership/allocation followup

Intercept the standard typed-array constructors in this isolated Node process
during one reference step and one already-created candidate step. Expect 15
Float64 allocations in the reference, totaling (8N+7E)*8 bytes; zero typed-array
constructions in the candidate. Restore constructors immediately afterward.
This measures neither V8 hidden allocations nor initialization/checkpoint cost.
Retain a checkpoint across four later steps and verify it and the constructor
input remain unchanged. A strictly positive1e-15 m³ film must survive and match
the dense reference exactly; no display/dry cutoff may erase it.
