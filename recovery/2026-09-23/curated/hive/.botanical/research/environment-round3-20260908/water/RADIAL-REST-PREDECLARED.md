...
First attempt u2702 exited1 at serialized-checkpoint comparisons. Owned u2706
proved an encoding defect: values were deeply equal, while raw constructor input
put geometry earlier than the pinned serializer's final geometry field. The cold
copy now emits fixed canonical top-level, geometry, face and event key order,
without erasing unknown fields before validation. One corrected serial boundary
plus radial/rest packet checks that representation fix; physical parameters,
criteria, numerical method and earlier failed evidence are unchanged.
