...
import {
  sourcePailContainer,
  sourceSuppliesContainer,
} from "../src/finite-sources.ts";
...
/**
 * A short, lawful fixture: the actual optimizer cuts one oak, then repairs
 * the cache with the earned wood. Low needs are an explicit prepared-state
...
export function prepareCareFixture() {
  ...
  for (let ticks = 0; ticks < 1_200 && state.felled < 1; ticks++)
    actualStep(state);
  ...
  for (let ticks = 0; ticks < 1_800 && !cache.repaired; ticks++)
    actualStep(state);
  assert.equal(cache.repaired, true, "real repair must open cache access");
  assert.ok(
    containerQuantity(
      state.materials,
      sourceSuppliesContainer(cache.id),
      "ration",
    ) >= 1,
  ...
  // This is the only prepared-state mutation. It does not alter custody or queue care.
  ...
  assert.equal(
    restored.actors.sedge.drafted,
    false,
    "Sedge remains an outsider",
  );
  assert.equal(restored.parties.home.members.includes("sedge"), false);
  assert.equal(
    restored.jobs.some((job) => job.kind === "care"),
    false,
  );
  assert.equal(
    restored.operations.some((operation) => operation.kind === "consume"),
    false,
  );
