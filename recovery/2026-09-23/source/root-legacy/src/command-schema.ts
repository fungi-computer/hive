...
const building = z.enum(["wall", "door", "roof", "bed", "shelf", "floor", "stair", "brew-station"]);
export const commandSchema = z.discriminatedUnion("kind", [
  z.object({ kind: z.literal("chop"), ...work, tree: id }).strict(),
  z.object({ kind: z.literal("dig"), ...work, ...cell }).strict(),
  z.object({ kind: z.literal("backfill"), ...work, ...cell }).strict(),
  z.object({ kind: z.literal("build"), ...work, ...cell, type: building, direction: z.number().int().min(0).max(1) }).strict(),
  ...
  z.object({ kind: z.literal("store"), party: id, actors: z.null(), lot: id, shelf: id }).strict(),
  ...
  z.object({ kind: z.literal("clear-spent-grain"), ...work, station: id }).strict(),
  z.object({ kind: z.literal("cancel"), ...scope, job: id }).strict(),
  z.object({ kind: z.literal("next"), ...scope, job: id }).strict(),
  z.object({ kind: z.literal("routine"), ...scope, enabled: z.boolean() }).strict(),
  z.object({ kind: z.literal("work"), ...scope, work: z.enum(["chop", "haul", "build", "garden", "craft"]), enabled: z.boolean() }).strict(),
  z.object({ kind: z.literal("draft"), party: id, actor: id }).strict(),
  z.object({ kind: z.literal("undraft"), party: id, actor: id }).strict(),
  z.object({ kind: z.literal("go"), party: id, actor: id, target: z.object(cell).strict() }).strict(),
