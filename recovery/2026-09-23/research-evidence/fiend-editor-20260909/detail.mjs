...
const targets=privateURL ? [['detail',privateURL]] : [];
...
try {
 for(const [label,url] of targets) {
  ...
  try {
   ...
   await page.waitForFunction(()=>document.body.innerText.includes('KESTREL'),null,{timeout:30000});
   await page.mouse.move(700,400);
   await page.locator('.Outliner .option').filter({hasText:'KESTREL'}).first().click();
   await page.screenshot({path:new URL('selected-properties.png',out).pathname});
   await page.getByRole('button',{name:'Feedback',exact:true}).first().click();
   await page.screenshot({path:new URL('feedback-panel.png',out).pathname});
 ...
 await writeFile(new URL('detail-observations.json',out),JSON.stringify({privateLinkFound:Boolean(privateURL),observations},null,2));
