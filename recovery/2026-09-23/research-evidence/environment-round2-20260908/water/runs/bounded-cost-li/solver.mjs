...
export function step(state,dt,{faceOrder,withdrawRate=0}={}) {
  ...
  for(let i=0;i<N;i++) {
    ...
    // Leave an explicitly measured Float64 roundoff reserve in exhausted donors.
    // This is internal to this LI discretization, never generic shared clipping.
  ...
  for(const f of faces)if(f.open) {
    ...
    if(hf>=.01) { // exclude vanishing film from advertised distribution; max film below is separate
      ...
      if(model==='li'){maxFr=Math.max(maxFr,fr);wet++;if(fr>.5)frAboveHalf++;if(fr>1)frAboveOne++;}
      maxGradient=Math.max(maxGradient,Math.abs(z[f.a]+h[f.a]-z[f.b]-h[f.b])/dx);
    }
  }
  if(model==='swe')for(let i=0;i<N;i++)if(h[i]>=.01){const fr=Math.hypot(mx[i],my[i])/(h[i]*Math.sqrt(G*h[i]));maxFr=Math.max(maxFr,fr);wet++;if(fr>.5)frAboveHalf++;if(fr>1)frAboveOne++;}
