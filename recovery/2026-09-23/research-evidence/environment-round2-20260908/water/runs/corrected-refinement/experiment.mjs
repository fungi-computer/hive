...
if(arg==='qualification') {
  ...
  for(const model of ['li','swe'])experiment(`rotation90-${model}`,()=>{const a=advance(makeFixture({model}),{end:120,retain:false}),b=advance(makeFixture({model,rotation:1}),{end:120,retain:false});let diff=0;const n=32;for(let y=0;y<n;y++)for(let x=0;x<n;x++)diff=Math.max(diff,Math.abs(b.state.V[y*n+x]-a.state.V[(n-1-x)*n+y]));return {summary:{mappedVolumeMaxDifference:diff,pass:diff<=1e-10,a:a.summary,b:b.summary},a:serialize(a.state),b:serialize(b.state)};});
} else if(arg==='radial') {
