...
import { createFiniteGas, VERSION, LIMITS } from './finite-gas.mjs';
import * as originalGas from '../finite-low-mach-v1/finite-gas.mjs';
import * as originalProjection from '../finite-low-mach-v1/projection.mjs';
import * as originalBoussinesq from '../finite-low-mach-v1/boussinesq-reference.mjs';

const root = fileURLToPath(new URL('.', import.meta.url));
const start = performance.now(), cpuStart = process.cpuUsage();
const paths = ['geometry-owner.mjs', 'projection.mjs', 'finite-gas.mjs', 'boussinesq-reference.mjs', 'qualify.mjs', 'CONTRACT.md', 'REFACTOR.md',
  '../finite-low-mach-v1/finite-gas.mjs', '../finite-low-mach-v1/projection.mjs',
  '../finite-low-mach-v1/geometry-owner.mjs', '../finite-low-mach-v1/boussinesq-reference.mjs',
  '../finite-low-mach-v1/handoff-inventory.json',
...
try {
  ...
  group('frozen evidence integrity and exact old/new physical outputs', () => {
    const originalRoot = root + '../finite-low-mach-v1/';
    const inventory = JSON.parse(readFileSync(originalRoot + 'handoff-inventory.json', 'utf8'));
    const verifyFrozen = () => {
      for (const [path, expected] of Object.entries(inventory))
        equal(createHash('sha256').update(readFileSync(originalRoot + path)).digest('hex'), expected);
    };
    verifyFrozen(); equal(VERSION, originalGas.VERSION); equal(LIMITS, originalGas.LIMITS);
    const exact = [];
    const pair = (id, geom, spec, callback) => {
      const old = originalGas.createFiniteGas(geom, spec), next = createFiniteGas(geom, spec);
      equal(old.identity, next.identity);
      const before = old.initial({ pressurePa: 100000, temperatureK: 300 });
      equal(next.initial({ pressurePa: 100000, temperatureK: 300 }), before);
      const a = callback(old, before), b = callback(next, before);
      equal(b, a); equal(next.diagnostics(), old.diagnostics());
      exact.push({ id, completeOutputEqual: true, workEqual: true });
      return b;
    };
    for (const dtMax of [4, 2, 1]) {
      const old = JSON.parse(readFileSync(originalRoot + `strip-dt${dtMax}.json`, 'utf8'));
      const next = JSON.parse(readFileSync(root + `strip-dt${dtMax}.json`, 'utf8'));
      equal(next.state, old.state); equal(next.receipt, old.receipt); equal(next.exactMassKg, old.exactMassKg);
      equal(next.result.work, old.result.work);
    }
    const small = smallGeometry();
    pair('rest', small, {}, (gas, s) => gas.advance(s, 10, { dtMax: 1, heatWatts: Array(small.n).fill(0) }));
    pair('zero-interval', smallGeometry(), {}, (gas, s) => gas.advance(s, 0, { heatWatts: Array(small.n).fill(0) }));
    pair('local-forward-Euler', smallGeometry(), {}, (gas, s) => gas.forwardEuler(s, 1, [540, 0, 0, 0, 0, 0]));
    const sg = gasGeometry(), source = sg.cells.map((c) => c.center[0] < 4 ? 1000 * sg.volume : 0);
    pair('heated-strip-complete-output', sg, {}, (gas, s) => gas.advance(s, 100, { dtMax: 2, heatWatts: source }));
    const reduced = pair('heated-strip-CFL-reductions', gasGeometry(), {},
      (gas, s) => gas.advance(s, 100, { dtMax: 50, heatWatts: source }));
    ok(reduced.retryReasons.courant > 0, 'equivalence exercises actual CFL reduction');
    const envelope = pair('intermediate-envelope-reductions', smallGeometry(), {}, (gas) => {
      const s = gas.initial({ pressurePa: 100000, temperatureK: 590 });
      return gas.advance(s, 1, { dtMax: 1, heatWatts: s.internalEnergyJ.map((u) => u / 590 * 9) });
    });
    ok(envelope.retryReasons.intermediateEnvelope > 0, 'equivalence exercises actual virtual-stage reduction');
    pair('helium-definition-existing-path', smallGeometry(), { speciesId: 'inert-helium' },
      (gas, s) => gas.advance(s, 5, { dtMax: 1, heatWatts: [54, 0, 0, 0, 0, 0] }));
    const old = originalGas.createFiniteGas(smallGeometry()), next = createFiniteGas(smallGeometry());
    const initial = old.initial({ pressurePa: 100000, temperatureK: 300 });
    const half = old.advance(initial, 5, { dtMax: 1, heatWatts: [540, 0, 0, 0, 0, 0] });
    const encoded = old.encode(half.state), restored = next.decode(encoded);
    equal(next.encode(restored), encoded);
    equal(next.advance(restored, 5, { dtMax: 1, heatWatts: [540, 0, 0, 0, 0, 0] }),
      old.advance(old.decode(encoded), 5, { dtMax: 1, heatWatts: [540, 0, 0, 0, 0, 0] }));
    cases.equivalence = { exact, recordedDt421StatesAndReceiptsEqual: true, crossVersionRestoreEqual: true };
    verifyFrozen();
  });

  group('exact old/new rejection class, message and unchanged input', () => {
    const g = smallGeometry(), old = originalGas.createFiniteGas(g), next = createFiniteGas(g);
    const initial = old.initial({ pressurePa: 100000, temperatureK: 300 });
    equal(next.initial({ pressurePa: 100000, temperatureK: 300 }), initial);
    const hot = old.initial({ pressurePa: 100000, temperatureK: 590 });
    equal(next.initial({ pressurePa: 100000, temperatureK: 590 }), hot);
    const nine = hot.internalEnergyJ.map((u) => u / 590 * 9), zero = Array(g.n).fill(0);
    const entries = [
      ['bulk-envelope', hot, (gas, s) => gas.advance(s, 1, { heatWatts: nine.map((x) => x * 2) })],
      ['accepted-work-exhaustion', hot, (gas, s) => gas.advance(s, 1, { heatWatts: nine, maxSteps: 1 })],
      ['nonrepresentable-clock', { ...initial, time: 2 ** 53 }, (gas, s) => gas.advance(s, .125, { heatWatts: zero })],
      ['work-admission', initial, (gas, s) => gas.advance(s, 3000, { heatWatts: zero })],
      ['nonfinite-source', initial, (gas, s) => gas.advance(s, 1, { heatWatts: zero.map((q, i) => i ? q : NaN) })],
      ['negative-interval', initial, (gas, s) => gas.advance(s, -1, { heatWatts: zero })],
      ['invalid-dt', initial, (gas, s) => gas.advance(s, 1, { dtMax: 0, heatWatts: zero })],
      ['foreign-identity', { ...initial, identity: 'other' }, (gas, s) => gas.advance(s, 1, { heatWatts: zero })],
      ['derived-pressure-stock', { ...initial, pressurePa: 100000 }, (gas, s) => gas.advance(s, 1, { heatWatts: zero })],
    ];
    const failures = [];
    for (const [id, input, run] of entries) {
      const before = JSON.stringify(input);
      const failure = (gas) => { try { run(gas, input); } catch (error) { return { name: error.name, message: error.message }; } throw new Error('expected rejection: ' + id); };
      const a = failure(old), b = failure(next); equal(b, a); equal(JSON.stringify(input), before);
      equal(next.diagnostics(), old.diagnostics()); failures.push({ id, ...b });
    }
    cases.equivalence.rejections = failures;
  });

  group('exact old/new shared projection and actual Boussinesq caller', () => {
    const g = smallGeometry(), predicted = [.01, -.015], beta = [.78, .98];
    const a = originalProjection.project(g, predicted, { faceCoefficient: beta });
    const b = project(g, predicted, { faceCoefficient: beta });
    equal(b, a); equal(projectionDiagnostics(g), originalProjection.projectionDiagnostics(g));
    const args = [
      [[NaN, 0], {}], [predicted, { faceCoefficient: [0, 1] }],
      [predicted, { targetVolumeFlux: [1, 0, 0, 0, 0, 0] }],
      [predicted, { targetVolumeFlux: [0, 0, 0, 1, 0, 0] }],
    ];
    for (const [velocity, options] of args) {
      const failure = (fn) => { try { fn(g, velocity, options); } catch (error) { return { name: error.name, message: error.message }; } throw new Error('expected projection rejection'); };
      equal(failure(project), failure(originalProjection.project));
      equal(projectionDiagnostics(g), originalProjection.projectionDiagnostics(g));
    }
    const room = geometry({ size: [4, 4, 4], spacing: [.5, .54, .5], periodic: [true, true, true], domainId: 'boussinesq-refactor-equality-v1' });
    const s = originalBoussinesq.initial(room), options = { dtMax: .05,
      forcingAt: () => ({ sources: [{ cell: 5, heatJS: 30, smokeKgS: 1e-6 }] }) };
    equal(legacy.initial(room), s);
    equal(legacy.advance(room, s, .15, options), originalBoussinesq.advance(room, s, .15, options));
    equal(legacy.diagnostics(room), originalBoussinesq.diagnostics(room));
    equal(legacy.transport(room, s, Array(room.faces.length).fill(0), .05, options.forcingAt()),
      originalBoussinesq.transport(room, s, Array(room.faces.length).fill(0), .05, options.forcingAt()));
    equal(legacy.diagnostics(room), originalBoussinesq.diagnostics(room));
    cases.equivalence.sharedProjectionAndBoussinesqCompleteEqual = true;
  });
