...
export function geometry({ nx, nz, dx, depth = 2, periodicX = false,
  ...
  assert(Number.isSafeInteger(nx) && nx>1 && Number.isSafeInteger(nz) && nz>1 &&
    Number.isFinite(dx) && dx>0 && Number.isFinite(depth) && depth>0, 'finite positive grid');
  ...
  g.identity=JSON.stringify({version:g.version,nx,nz,dx,depth,periodicX,periodicZ,
...
function neighbor(g, velocity, f, ox, oz) {
  ...
  if (k >= 0) {
    if (!tangential) return velocity[k];
    ...
    return fraction * velocity[k] - (1-fraction)*center;
  }
...
export function advance(g, input, interval, { dtMax = .05, forcingAt = () => ({}), events = [] } = {}) {
  ...
  return { state, receipt, rejected, maxDivergence, maxCourant };
}
