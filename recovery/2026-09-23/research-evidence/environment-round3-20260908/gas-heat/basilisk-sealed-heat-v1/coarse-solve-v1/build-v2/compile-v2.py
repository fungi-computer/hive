...
checkpoint=json.loads((ROOT/'EXECUTABLE-CHECKPOINT-v2.json').read_text())
...
shutil.copyfile(ROOT/'EXECUTABLE-CHECKPOINT-v2.json',OUT/'EXECUTABLE-CHECKPOINT-v2.json')
env=os.environ.copy();env['BASILISK']=str(SRC);env['OMP_NUM_THREADS']='1'
commands=[
    [str(SRC/'qcc'),'-O2','-Wall','-disable-dimensions','algebra.c','-o','algebra','-lm'],
    [str(SRC/'qcc'),'-O2','-Wall','-disable-dimensions','-events','-source','sealed-heat.c'],
    [str(SRC/'qcc'),'-O2','-Wall','-disable-dimensions','sealed-heat.c','-o','sealed-heat','-lm']]
