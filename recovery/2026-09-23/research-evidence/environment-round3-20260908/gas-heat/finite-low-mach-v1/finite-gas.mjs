...
export function createFiniteGas(g, { speciesId = 'inert-air' } = {}) {
  ...
  function advance(input, interval, { dtMax = 1, heatWatts, maxSteps = LIMITS.maxSteps } = {}) {
    ...
    let state = input, rejected = 0, lastStage = null, maxConstraint = 0, maxEOS = 0, maxCourant = 0, maxMach = 0;
    let maxAcceptedTemperatureK = Math.max(...fields(input).temperature);
    const retryReasons = { courant: 0, intermediateEnvelope: 0 };
    const massTransferKg = new Float64Array(g.faces.length), enthalpyTransferJ = new Float64Array(g.faces.length), heatTransferJ = new Float64Array(g.n);
    while (state.time < end) {
      assert(state.steps - input.steps < maxSteps, 'accepted-step work budget exhausted; input uncommitted');
      let dt = Math.min(dtMax, end - state.time), a, b;
      ...
      const acceptedFields = validate(average); state = freezeState(average);
      maxAcceptedTemperatureK = Math.max(maxAcceptedTemperatureK, ...acceptedFields.temperature);
      for (const [key, output] of [['massTransferKg', massTransferKg], ['enthalpyTransferJ', enthalpyTransferJ], ['heatTransferJ', heatTransferJ]])
        for (let i = 0; i < output.length; i++) output[i] += (a.receipt[key][i] + b.receipt[key][i]) / 2;
      for (const stage of [a, b]) {
        maxConstraint = Math.max(maxConstraint, stage.receipt.constraintError);
        maxEOS = Math.max(maxEOS, stage.receipt.eosError); maxCourant = Math.max(maxCourant, stage.receipt.courant);
        maxMach = Math.max(maxMach, stage.receipt.maxMach);
      }
      lastStage = b.receipt;
    }
    return { state, receipt: { start: input.time, end: state.time, massTransferKg: Array.from(massTransferKg),
      enthalpyTransferJ: Array.from(enthalpyTransferJ), heatTransferJ: Array.from(heatTransferJ) },
