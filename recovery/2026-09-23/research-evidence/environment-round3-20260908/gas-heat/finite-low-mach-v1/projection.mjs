...
function workspace(g) {
  ...
  for (let i = 0; i < g.n; i++) {
    ...
    const pending = [i], cells = []; let open = false;
    ...
    assert(cells.filter((at) => g.fixed[at]).length === (open ? 0 : 1), 'geometry pressure pins match component');
    components.push(Object.freeze({ cells: Object.freeze(cells), open }));
  }
  const cell = () => new Float64Array(g.n), face = () => new Float64Array(g.faces.length);
  const w = { components: Object.freeze(components), diagonal: cell(), weight: face(), beta: face(),
...
export function project(g, predicted, { targetVolumeFlux, faceCoefficient } = {}) {
  ...
  // Rebuild numeric coefficients at EVERY solve/RK stage. Geometry topology alone
  // is reusable when density changes; the old geometry diagonal is not consulted.
  w.diagonal.fill(0);
  for (const f of g.faces) {
  ...
  const linearResidual = maxAbs(w.r);
