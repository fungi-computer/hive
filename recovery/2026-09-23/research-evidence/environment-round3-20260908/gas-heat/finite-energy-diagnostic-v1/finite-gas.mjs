...
import { gravityEnergy, averagingEnergy, acceptedEnergy } from './energy-diagnostics.mjs';
...
export function createFiniteGas(inputGeometry, { speciesId = 'inert-air', heatWatts, gravity, bodyForce = emptyForce } = {}) {
  ...
  function euler(s, dt) {
    ...
    const energyDiagnostic = { forceTime: s.time, constraintTime: state.time, interval: dt,
      transport: moved.energyDiagnostic, projection: projected.energyDiagnostic,
      gravity: gravityEnergy(g, s.massKg, before.velocity, thermal.receipt.massTransferKg, dt, acceleration,
        checked.potentialEnergyJ - before.potentialEnergyJ, moved.energyDiagnostic.body.oldVelocityWorkJ) };
    return { state: freezeState(state), energyDiagnostic, receipt: { ...thermal.receipt, ...moved.receipt,
  ...
  function acceptTrial(accepted, { dt, a, b, rejected, retryReasons }) {
    ...
    const oldK = kineticEnergy(g, before.massKg, before.faceMomentumKgMS), secondK = kineticEnergy(g, b.state.massKg, b.state.faceMomentumKgMS);
    const averageDiagnostic = averagingEnergy(dual, dualMasses(g, before.massKg), dualMasses(g, b.state.massKg),
      before.faceMomentumKgMS, b.state.faceMomentumKgMS, oldK, secondK, fields.kineticEnergyJ);
    accepted.energyDiagnostics.push(acceptedEnergy(a.energyDiagnostic, b.energyDiagnostic,
      { ...finalProjection.energyDiagnostic, weight: 1, constraintTime: state.time, kind: 'endpoint-constraint-correction' },
      averageDiagnostic, { start: before.time, end: state.time, rejected,
        kineticChangeJ: checked.kineticEnergyJ - oldK,
        potentialChangeJ: checked.potentialEnergyJ - potentialEnergy(before.massKg),
        mechanicalChangeJ: checked.kineticEnergyJ - oldK + checked.potentialEnergyJ - potentialEnergy(before.massKg) }));
  ...
  function advance(input, interval, options = {}) {
    ...
    return { state, energyDiagnostics: accepted.energyDiagnostics, receipt: { start: input.time, end: state.time, ...transfers, ...mechanical }, rejected, retryReasons, lastStepPressure,
