...
out = root.parent / 'result-v2'
out.mkdir(exist_ok=False)

def sha(path):
    return hashlib.sha256(path.read_bytes()).hexdigest()

names = ['SOURCE-FIT.md', 'FIRST-SHAPE.md', 'CAPTURE.md', 'geometry.h', 'continuum.h',
...
record = {
    ...
    'controls': {'nonnegativeInterpolatedVolume': 'all fixed queries >= 0; failed flag retained even when native consumer ignores it', 'meshSdfM': 1e-12, 'fineOffSurfaceSdfDivR': .01,
                 'fineVolumeInterpolationDivR3': .01, 'rule30Rule50VolumeDivR3': .01,
                 'contactPositionM': 1e-14, 'positiveState': 'exact', 'restoration': 'exact'},
    'preservedFirstFailure': sha(root.parent / 'result-v1/result.json'),
    'captureCorrection': 'same numerical fields; nonnegative volume remains a false control when violated, but observations complete',
    ...
    'frozenColdOracle': sha(root.parent.parent / 'boundary-consistency-v1/result-v1/result.json'),
    'frozenPhysicalFailure': sha(root.parent.parent / 'physical-voxel-v1/result-v1/result.json'),
    'frozenLibraries': {name: sha(root.parent.parent / 'stable-order-v1' / name) for name in
...
try:
    ...
    prior = (root.parent.parent / 'boundary-consistency-v1/result-v1/initialization.csv').read_text().splitlines()
