...
out = root / (sys.argv[1] if len(sys.argv) > 1 else 'restart-v1')
...
record = {
    'scope': 'same native binary, two fresh processes; one fixed tank, fixed dt, Z-sort disabled; no game/WASM/portable-save claim',
    ...
    'sourceSha256': {name: sha(root / name) for name in
                     ['reference.cpp', 'reference-checkpoint.h', 'reference', 'compile-reference.py', 'run-restart.py']},
    'results': [],
}
