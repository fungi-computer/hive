...
BUILD = ROOT / "build-v1"
OUT = ROOT / "result-v1"
COMPARATOR = ROOT.parent / "reporting-v2/source/compare-wave.mjs"
assert hashlib.sha256(COMPARATOR.read_bytes()).hexdigest() == "e6e1fe204b72af072d6f15a62a80980cb1258517c31cdfb3e925cd04d050f2f8"
...
for name in ("wave.c", "wave-observation.h", "wave-reduced-observation.h"):
...
for case in cases:
    try:
        accounting.append(inspect_force(case))
    except (KeyError, TypeError, ValueError, ArithmeticError) as error:
        accounting.append({"case": case["caseName"], "capturedAccountingPassed": False,
                           "readerError": str(error), "rawCapturePreserved": True})
accounting_ok = len(cases) == 2 and all(case["capturedAccountingPassed"] for case in accounting)
...
summary = {"invocationId": receipt["invocationId"], "comparisonExitCode": compare_code,
           "capturedForceAccountingPassed": accounting_ok,
