...
function nativeControls(capture, rows, grid) {
  ...
  for (let j = 0; j < steps.length; j++) {
    ...
    clocks &&= row.step === j + 1 && row.vofInvocationCount === j + 1 &&
      finite(dt) && dt > 0 && dt <= oracle.periodS / 128 * (1 + 1e-12) &&
  ...
  const rowStates = rows.every(r => r.case === capture.caseName && r.valid === true && r.clockValid === true && r.solveValid === true &&
    ...
    near(r.waterM3PerMetre, .54, 5.4e-7) && near(r.airM3PerMetre, .54, 5.4e-7) &&
    r.activeTransportBudgetM3PerM === 5.4e-7 &&
    r.legacyStockWithinOriginalLimit === (near(r.waterM3PerMetre, .54, 1e-10) && near(r.airM3PerMetre, .54, 1e-10)) &&
  ...
  return {
    ...
    oneFixedDefinition: definitions.length === 1 && definition.case === capture.caseName &&
      ...
      near(definition.periodSeconds, oracle.periodS) && near(definition.dtCapSeconds, oracle.periodS / 128) &&
    ...
    oneInitial: rows.filter(r => r.step === 0).length === 1 && initial.step === 0 &&
      ...
      near(initial.waterM3PerMetre, .54, 1e-10) && near(initial.airM3PerMetre, .54, 1e-10),
...
function compareCase(capture) {
  ...
  try {
    ...
    return { caseName: capture.caseName, passed: all(controls) && all(accuracy), controls, accuracy,
      coverage,
      legacyDiagnostic: { scope: "Preserved original absolute stock gate, excluded from new packet acceptance",
        limitM3PerM: 1e-10, passed: rows.every(r => r.legacyStockWithinOriginalLimit === true),
        firstFailingStep: rows.find(r => r.legacyStockWithinOriginalLimit === false)?.step ?? null,
        predecessorRun: "u3386.scope", predecessorPassed: false },
      transportBudget: { initialNominalWaterM3PerM: .54, fraction: 1e-6, budgetM3PerM: 5.4e-7,
        horizon: "this fixed one-period reference only; not a game or 30-minute policy" },
