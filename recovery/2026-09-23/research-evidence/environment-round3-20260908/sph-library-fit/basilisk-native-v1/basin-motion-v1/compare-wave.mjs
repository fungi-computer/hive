...
function compareCase(capture) {
  ...
  try {
    ...
    const accuracy = {
      ...
      returnedAmplitude: returnAmplitudeError <= grid.returned,
    ...
    return { caseName: capture.caseName, passed: all(controls) && all(accuracy), controls, accuracy,
      limits: grid, initialAmplitudeError, returnAmplitudeError, temporal, maxima,
...
const refinement = finite(coarseRms) && finite(fineRms) &&
...
const result = { scope: 'Fixed 2D linear-wave comparison with finite-amplitude and nominal-stage limitations. Energy is a staggered diagnostic.',
  ...
  sourceCapture: process.argv[2], cases, refinement: { passed: refinement, coarseRms, fineRms,
