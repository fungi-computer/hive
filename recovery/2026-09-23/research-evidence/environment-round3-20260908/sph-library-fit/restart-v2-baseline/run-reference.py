...
for mode in ['freefall', 'compression', 'tank']:
    result = subprocess.run([str(root / 'reference'), mode], cwd=root,
                            ...
                            timeout=15)
    (root / f'{mode}-v2.stdout').write_text(result.stdout)
    (root / f'{mode}-v2.stderr').write_text(result.stderr)
...
(root / 'reference-v2.json').write_text(json.dumps(record, indent=2)+'\n')
