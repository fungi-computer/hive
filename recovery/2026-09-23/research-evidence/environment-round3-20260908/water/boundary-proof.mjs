...
experiment('private-input-borrowed-view-and-retained-checkpoint', () => {
  ...
  const reversed = Object.fromEntries(Object.entries(clone(wet)).reverse());
  reversed.geom = Object.fromEntries(Object.entries(reversed.geom).reverse());
  reversed.geom.faces = reversed.geom.faces.map(f => Object.fromEntries(Object.entries(f).reverse()));
  reversed.events = Object.fromEntries(Object.entries(reversed.events).reverse());
  assert.equal(stateBytes(createStepper(reversed).checkpoint()), before, 'field insertion order must not affect canonical encoding');
  ...
  return { inputIndependent: true, checkpointGeometryAndHistoryIndependent: true, viewsReadOnly: true, retainedAcrossLaterSteps: true, canonicalFieldOrderIndependent: true };
});
