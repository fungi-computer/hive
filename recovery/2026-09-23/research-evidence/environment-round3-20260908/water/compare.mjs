...
const hashes = {};
for (const name of ['candidate.mjs', 'admission.mjs', 'fixtures.mjs', 'compare.mjs', 'PREDECLARED.md']) {
...
function compare(initial, { end = 8, maxDt = .05, edits = false, withdrawRate = 0, gateAt = 2, digAt = 5 } = {}) {
  ...
  while (reference.time < end - 1e-10) {
    if (edits && !reference.events.gate && reference.time >= gateAt - 1e-10) {
      reference = edit(reference, 'gate-open'); dense.edit('gate-open'); active.edit('gate-open');
      if (resumed) resumed.edit('gate-open');
    }
    if (edits && !reference.events.dig && reference.time >= digAt - 1e-10) {
      const before = reference;
      reference = edit(reference, 'dig-pond'); dense.edit('dig-pond'); active.edit('dig-pond');
      if (resumed) resumed.edit('dig-pond');
    ...
    const ownerDt = active.stableDt(maxDt);
    assert.equal(ownerDt, stableDt(reference, maxDt), 'owner stability matches reference');
    assert.equal(dense.stableDt(maxDt), ownerDt, 'dense workspace stability');
    let dt = Math.min(ownerDt, end - active.time);
