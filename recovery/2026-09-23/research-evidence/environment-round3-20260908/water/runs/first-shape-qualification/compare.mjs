...
function compare(initial, { end = 8, maxDt = .05, edits = false, withdrawRate = 0 } = {}) {
  ...
  let gateBeforeOpen = 0;
  while (reference.time < end - 1e-10) {
    if (edits && !reference.events.gate && reference.time >= 2 - 1e-10) {
    ...
    if (edits && !reference.events.dig && reference.time >= 5 - 1e-10) {
    ...
    if (edits && !reference.events.gate) dt = Math.min(dt, 2 - reference.time);
    if (edits && !reference.events.dig) dt = Math.min(dt, 5 - reference.time);
  ...
  return { steps, maxBalance, minDepth, gateBeforeOpen, final: serialize(reference), metrics: metrics(reference), dense: dense.stats(), active: active.stats(), restartExact: true, retainedSnapshotTime: JSON.parse(snapshot).time };
}
