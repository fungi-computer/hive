...
experiment('owned-supported-edits', () => {
  ...
  for (const kind of ['gate-open', 'dig-pond']) {
    ...
    const expected = step(reference, .025, { withdrawRate: .06 });
    sameResult(expected, solver.step(.025, { withdrawRate: .06 })); reference = expected.state;
  }
...
experiment('wet-reference-continuation-and-history', () => {
  ...
  for (let k = 0; k < 100; k++) {
    ...
    const dt = stableDt(reference, .05), expected = step(reference, dt, { withdrawRate: .06 });
  ...
  return { intervals: 100, completeStateReceiptsDiagnosticsExact: true, restartExact: true,
    withdrawnDuringContinuation: reference.collected - collectedBefore, qDiscardHasNoEffect: true, lostHistoryL1, final: solver.checkpoint() };
});
...
experiment('rejection-retry-and-step-allocation', () => {
  ...
  let actual;
  try { actual = solver.step(.025); } finally { for (const [key, Type] of originals) globalThis[key] = Type; }
  assert.equal(count, 0); assert.equal(bytes, 0); sameResult(step(initial, .025), actual);
  return { rejectedCalls: 4, canonicalStateUnchanged: true, exactRetry: true, typedArraysDuringStep: count, typedBytesDuringStep: bytes };
});
