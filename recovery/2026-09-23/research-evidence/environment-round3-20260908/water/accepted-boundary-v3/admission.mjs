...
function copyGeometry(g) {
  return { ...g, z: Array.from(g.z), solid: Array.from(g.solid), region: Array.from(g.region),
    faces: g.faces.map(f => ({ ...f, collinear: Array.from(f.collinear), transverse: Array.from(f.transverse) })),
    xids: Array.from(g.xids), yids: Array.from(g.yids) };
}

export function retainedCheckpoint(s) {
  return { ...s, V: Array.from(s.V), q: Array.from(s.q), mx: Array.from(s.mx), my: Array.from(s.my),
    events: { ...s.events }, geom: copyGeometry(s.geom) };
}
