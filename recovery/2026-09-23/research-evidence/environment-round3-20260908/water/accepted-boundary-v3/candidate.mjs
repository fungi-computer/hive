...
import { G, edit as referenceEdit, stableDt as referenceStableDt } from './reference/solver.mjs';
...
export function createStepper(initial, options = {}) {
  ...
  const exposeState = (s, geometry) => Object.freeze({ ...s,
    V: fieldView(s.V), q: fieldView(s.q), mx: fieldView(s.mx), my: fieldView(s.my),
    geom: geometry, events: Object.freeze({ ...s.events }),
  });
  let inspectedSource = null, inspectedValue = null;
  function inspectState(s = state, geometry = exposedGeometry) {
    if (inspectedSource !== s) { inspectedSource = s; inspectedValue = exposeState(s, geometry); }
  ...
  function step(dt, options = {}) {
    ...
    const committed = state, committedGeometry = exposedGeometry;
    return {
      // Study-only diagnostic inspection. Ordinary callers use owner.stableDt,
      // scalar time/interval and copied checkpoints, never per-index facades.
      get state() { return inspectState(committed, committedGeometry); },
      interval: Object.freeze(interval), exchanges: exposedExchanges, withdrawals: exposedWithdrawals,
  ...
  return {
    ...
    // Kept only for dense-reference comparisons, not an intended bulk data API.
    get state() { return inspectState(); },
    stableDt(maxDt = .1) {
      if (!(typeof maxDt === 'number' && Number.isFinite(maxDt) && maxDt > 0)) throw Error('invalid maximum dt');
      return referenceStableDt(state, maxDt);
    },
    step,
    ...
    edit(kind) {
      ...
      if (state.events[event]) return inspectState();
