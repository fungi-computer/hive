...
function normalizeWetBoundaryHeads(g, closed) {
  const normalizations = [];
  for (const [i, node] of g.nodes.entries()) {
    if (node.kind === 'soil' || closed.mass[i] === 0) continue;
    const previousHeadM = closed.heads[i];
    closed.heads[i] = closed.mass[i] / (g.rho * node.area);
    normalizations.push({ node: i, massKg: closed.mass[i], previousHeadM,
...
function boundaryFacts(g, closed) {
  return g.nodes.flatMap((node, i) => {
    ...
    const depthM = closed.mass[i] / (g.rho * node.area);
    return [{ node: i, massKg: closed.mass[i], depthM,
      headM: closed.heads[i], gapM: depthM - closed.heads[i] }];
  });
}
...
export function solveStep(g, oldMass, dt, work) {
  ...
  const wetBoundaryNormalizations = normalizeWetBoundaryHeads(g, closed);
  try {
  ...
  return { mass: closed.mass, heads: closed.heads, metrics,
    closure: { node: closed.anchor, correctionKg: closed.correctionKg, wetBoundaryNormalizations } };
  } catch (error) {
    error.candidateBoundaries = boundaryFacts(g, closed);
    error.wetBoundaryNormalizations = wetBoundaryNormalizations;
    throw error;
  }
}
