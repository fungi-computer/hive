import { worldIdentity, createVoxelWorld, MATERIAL } from '../../worldgen/coordinate-hash-v1/voxel-world.mjs';
import { createWorldSpec, sampleTerrain } from '../../worldgen/coordinate-hash-v1/terrain.js';
import { createVolume } from '../volume-v1/volume.mjs';
import { createVolumeGeometry } from '../volume-v1/geometry.mjs';
...
export function fixedExcavationFixture({ soleTargetAnchor = false } = {}) {
  ...
  for (const x of [-1, 0, 1]) for (const z of [127, 128, 129]) {
    ...
    for (const y of [bedLevel - 2, bedLevel - 1]) {
      requireCondition(world.read({ x, y, z }) === MATERIAL.soil, 'fixed fixture has its two real generated soil layers');
  ...
  const descriptor = { regionId: 'generated-pit-region', revision: world.describe().revision,
    spacingM: [1, .54, 1], exterior: 'closed', definitions: [REFERENCE_SOIL],
  ...
  const waterTableYM = target[1] * .54 + .15;
