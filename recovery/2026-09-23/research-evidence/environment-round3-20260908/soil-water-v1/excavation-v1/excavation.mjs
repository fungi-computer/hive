...
function validateExcavationHistory(config, state, facts) {
  ...
  requireCondition(previousWorld.read(xyz(at)) === 1, 'recorded edit must actually remove world soil');
...
function admitTarget(checked, command) {
  ...
  requireCondition(target && checked.world.read(xyz(command.at)) === 1, 'excavation targets one owned world-soil voxel');
