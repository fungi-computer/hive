import { hashString, latticeHash3 } from "../coordinate-hash-v1/lattice-hash.mjs";
...
import { createWorldSpec, sampleTerrain, floorDiv, mod } from '../coordinate-hash-v1/terrain.js';
import { createCaveFeatures, CAVE_FEATURE_RECIPE } from '../coordinate-hash-v1/features.mjs';

const RECIPE = Object.freeze({
  codecSchema: 2,
  heightSource: '530464448725cacb73836f34c2c48ddbf3e4a8c0d8a353498e7fe6da03f45bdf',
  voxelRecipe: 'height-sea-connected-caves-v3',
  features: CAVE_FEATURE_RECIPE,
  units: Object.freeze({ horizontalMetres: 1, verticalMetres: 0.54, voxelsPerStorey: 4 }),
  bounds: Object.freeze({ minX: -2048, maxX: 2048, minZ: -2048, maxZ: 2048, minY: -64, maxY: 64 }),
  brickSide: 16,
  residentBrickCap: 8,
  cave: Object.freeze({
    ...
    macro: Object.freeze({ horizontalMetres: 32, verticalMetres: 18, weight: 0.78, salt: 'cave-macro' }),
    detail: Object.freeze({ horizontalMetres: 11, verticalMetres: 7, weight: 0.22, salt: 'cave-detail' }),
    voidThreshold: 0.42,
...
function checkCell(at) {
  ...
  if (at.x < bounds.minX || at.x >= bounds.maxX || at.y < bounds.minY || at.y >= bounds.maxY || at.z < bounds.minZ || at.z >= bounds.maxZ)
...
function compareCells(left, right) { return left.x - right.x || left.z - right.z || left.y - right.y; }
...
function signedNoise3D(fieldHash, x, y, z, salt) {
  ...
  const at = (dx, dy, dz) => caveLattice(fieldHash, x0 + dx, y0 + dy, z0 + dz, salt) * 2 - 1;
...
function checkBrickAddress(input) {
  ...
  checkCell(origin);
  checkCell({ x: origin.x + SIDE - 1, y: origin.y + SIDE - 1, z: origin.z + SIDE - 1 });
  return origin;
}

function readCheckpoint(identity, checkpoint, baseAt) {
  if (checkpoint === null) return { revision: 0, entries: [] };
  if (!checkpoint || checkpoint.schema !== RECIPE.codecSchema || !equal(checkpoint.identity, identity))
...
export function createVoxelWorld(identity, { checkpoint = null, maxResidentBricks = RECIPE.residentBrickCap } = {}) {
  const expected = worldIdentity({ worldId: identity?.worldId, spaceId: identity?.spaceId, seed: identity?.base?.heightSeed });
  ...
  const features = createCaveFeatures({ spec, units: RECIPE.units, bounds: RECIPE.bounds });
  const changes = new Map(), byBrick = new Map(), cache = new Map();
  let pointReads = 0, generatedPointReads = 0;
  let revision = 0, generatedBricks = 0, evictions = 0, heightSamples = 0, caveMetricEvaluations = 0;

  function columnAt(x, z) {
    // Exact physical height only; the display sampleCell would also query four neighbors.
    const cell = sampleTerrain(spec, x, z, 1); heightSamples += 1;
  ...
  function caveMetric(at) {
    ...
    const scaled = (field) => signedNoise3D(
      fieldHash,
    ...
    return RECIPE.cave.macro.weight * scaled(RECIPE.cave.macro) + RECIPE.cave.detail.weight * scaled(RECIPE.cave.detail);
  }
  ...
  function baseMaterialAt(at, column = columnAt(at.x, at.z)) {
    ...
    return caveMetric(at) >= RECIPE.cave.voidThreshold ? MATERIAL.air : MATERIAL.stone;
  }
  ...
  function removeChange(at) {
    const key = cellKey(at), brick = brickKey(at), entries = byBrick.get(brick);
    changes.delete(key); entries?.delete(key);
    if (entries?.size === 0) byBrick.delete(brick);
  }
  function decodeOrigin(origin) {
    const key = brickKey(origin), hit = cache.get(key);
    if (hit) { cache.delete(key); cache.set(key, hit); return hit; }
    const material = new Uint8Array(VOLUME);
    for (let localZ = 0; localZ < SIDE; localZ += 1) for (let localX = 0; localX < SIDE; localX += 1) {
      const x = origin.x + localX, z = origin.z + localZ, column = columnAt(x, z);
      for (let localY = 0; localY < SIDE; localY += 1) {
        const y = origin.y + localY;
        material[(localY * SIDE + localZ) * SIDE + localX] = baseMaterialAt({ x, y, z }, column);
      }
    }
    for (const entry of byBrick.get(key)?.values() ?? []) material[index(entry)] = entry.material;
    generatedBricks += 1;
    cache.set(key, material);
    while (cache.size > maxResidentBricks) { cache.delete(cache.keys().next().value); evictions += 1; }
    return material;
  }
  function decode(at) { return decodeOrigin({ x: floorDiv(at.x, SIDE) * SIDE, y: floorDiv(at.y, SIDE) * SIDE, z: floorDiv(at.z, SIDE) * SIDE }); }
  ...
  const restored = readCheckpoint(identity, checkpoint, (at) => baseMaterialAt(at));
  ...
  return Object.freeze({
    ...
    readBrick(brick) {
      const origin = checkBrickAddress(brick), material = decodeOrigin(origin);
      // A caller gets a disposable snapshot, never the canonical cache array.
      return { key: brickKey(origin), origin, side: SIDE, material: material.slice() };
    },
    describe() {
      return {
        identity: clone(identity), revision,
        residentProjectionCapBytes: maxResidentBricks * VOLUME,
        coldBrickBudget: { cells: VOLUME, heightColumns: SIDE * SIDE, caveMetricsAtMost: VOLUME, featurePrimitiveTestsAtMost: VOLUME * 4 },
        sparseOverlay: 'actual count/bytes reported by stats; no total overlay admission limit in this study',
      };
    },
    edit({ expectedRevision, cells }) {
      ...
      for (const input of cells) {
        ...
        seen.add(cellKey(at));
        if (readPoint(at) !== expectedMaterial) return { ok: false, reason: 'cell-changed', revision };
        if (expectedMaterial !== material) next.push({ entry: { ...at, material, revision: revision + 1 }, base: baseMaterialAt(at) });
      }
    ...
    save,
    evictAll() { evictions += cache.size; cache.clear(); features.evictAll(); },
    stats() {
