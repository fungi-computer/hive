...
import { compensatedSum, requireCondition } from '../../column-v1/source-v2/soil.mjs';
import { key, xyz, same, ordered, immutable, exactFields, coordinate, restoreWorld,
  assertRegionWorld, pitContacts, assertVented, excavateWorld } from './world-binding.mjs';

const VERSION = 'one-vented-soil-excavation-with-finite-pit-v1';
...
function validateLedger(state, facts) {
  ...
  for (const entry of state.exports) {
    ...
    requireCondition(typeof entry.id === 'string' && typeof entry.fromNodeId === 'string' &&
      ...
      entry.sourceVoxelM3 === .54, 'finite named wet-spoil water and source volume');
  }
...
function validateExcavationHistory(config, state, facts) {
  ...
  if (currentTarget) requireCondition(currentTarget.material === 0 && currentTarget.revision === state.world.revision,
  ...
  if (previousTarget !== null) {
    ...
    requireCondition(matchesAt(previousTarget, at) && previousTarget.material === 1,
  ...
  requireCondition(previousWorld.readPoint(xyz(at)) === 1, 'recorded edit must actually remove world soil');
...
function validate(config, identity, state) {
  ...
  const balance = validateLedger(state, facts);
...
function admitTarget(checked, command) {
  ...
  requireCondition(target && checked.world.readPoint(xyz(command.at)) === 1, 'excavation targets one owned world-soil voxel');
...
export function createExcavationAdapter(input) {
  ...
  function excavate(input, rawCommand) {
    ...
    const state = own({ ...input, world: checked.world.save(), soilGeometry: rebuilt.owner.geometry,
      ...
      exports: [{ id: `wet-spoil:${command.operationId}`, fromNodeId: stock.nodeId,
        soilId: target.soilId, waterKg: stock.massKg, sourceVoxelM3: .54 }],
  ...
  return Object.freeze({ identity, initial, excavate,
    ...
    decode: raw => {
      ...
      const state = JSON.parse(raw); validate(config, identity, state); return own(state);
    },
