...
export function pitNode(input, attached, cells) {
  const at = input.at, baseYM = at[1] * .54, rimYM = (at[1] + 1) * .54;
  ...
  requireCondition(floorCell.maxHeadM >= .54, 'pit rim lies within its floor retention envelope');
  requireCondition(floor.face.centerM[1] === baseYM && floor.face.areaM2 === 1 &&
    sides.every(p => p.face.centerM[1] === (at[1] + .5) * .54 && p.face.areaM2 === .54),
    'actual pit floor and side dimensions');
  return Object.freeze({ id: `reservoir:${input.id}`, reservoirId: input.id, kind: 'pit', at,
    elevationM: baseYM, baseYM, rimYM, areaM2: 1, portCount: attached.length,
    ...
    minHeadM: floorCell.minHeadM, maxHeadM: .54,
    minMassKg: 0, maxMassKg: 1000 * .54 });
}
