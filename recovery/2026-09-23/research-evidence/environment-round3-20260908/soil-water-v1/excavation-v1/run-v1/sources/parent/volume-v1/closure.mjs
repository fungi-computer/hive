...
function mobileAnchor(g, massKg) {
  ...
  for (const kind of ['reservoir', 'soil']) for (const i of order) {
...
function normalizeWetHeads(g, closed) {
  ...
  for (const [i, node] of g.nodes.entries()) {
    if (node.kind !== 'reservoir' || closed.massKg[i] === 0) continue;
