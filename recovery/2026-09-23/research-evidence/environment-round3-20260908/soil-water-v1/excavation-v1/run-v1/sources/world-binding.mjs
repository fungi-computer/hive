import { createVoxelWorld, MATERIAL } from '../../worldgen/coordinate-hash-v1/voxel-world.mjs';
...
export function restoreWorld(identity, checkpoint) {
  ...
  exactFields(checkpoint, ['schema', 'identity', 'revision', 'changes'], 'world checkpoint fields');
  for (const change of checkpoint.changes)
    exactFields(change, ['x', 'y', 'z', 'material', 'revision'], 'world change fields');
  return createVoxelWorld(identity, { checkpoint });
}
export function assertRegionWorld(world, descriptor) {
  ...
  for (const cell of descriptor.cells)
    requireCondition(world.read(xyz(coordinate(cell.at))) === MATERIAL.soil,
...
export function pitContacts(world, descriptor, at) {
  ...
  for (let axis = 0; axis < 3; axis++) for (const sign of [-1, 1]) {
    ...
    const material = world.read(xyz(neighbor)), faceAt = [...at];
