import { createSoil, REFERENCE_SOIL, requireCondition } from '../column-v1/source-v2/soil.mjs';
...
const SPACING = Object.freeze([1, 0.54, 1]);
...
function coordinate(at) {
  requireCondition(Array.isArray(at) && at.length === 3 && at.every(Number.isSafeInteger),
    'signed integer voxel coordinate');
  requireCondition(at[0] >= -2048 && at[0] < 2048 && at[1] >= -64 && at[1] < 64 &&
    at[2] >= -2048 && at[2] < 2048, 'coordinate in current finite voxel recipe bounds');
...
function definitions(inputs) {
  ...
  for (const input of inputs) {
    ...
    const ordered = Object.fromEntries(Object.keys(REFERENCE_SOIL).map(k => [k, input[k]]));
    owners[input.id] = createSoil(ordered);
  }
  return Object.freeze(owners);
}

function soilNodes(inputs, soils) {
  ...
  const nodes = inputs.map(input => {
    ...
    const volumeM3 = 0.54, rho = soil.definition.densityKgM3;
...
function faceDescriptor(at, axis, sign) {
...
function reservoirInputs(inputs) {
  ...
  return inputs.map(input => {
    ownKeys(input, ['id', 'areaM2'], 'finite reservoir definition fields');
...
function compilePorts(inputs, lookup, reservoirs) {
  ...
  for (const input of inputs) {
    ...
    const face = faceDescriptor(at, axis, sign);
...
function reservoirNodes(inputs, ports, cells) {
  return inputs.map(input => {
...
function compileFaces(cells, nodes, lookup, ports, closedInputs) {
  ...
  for (const [i, cell] of cells.entries()) for (let axis = 0; axis < 3; axis++) for (const sign of [-1, 1]) {
    const face = faceDescriptor(cell.at, axis, sign);
...
export function createVolumeGeometry(input) {
  ...
  requireCondition(input.version === undefined || input.version === 'rigid-soil-voxel-graph-v1',
  ...
  requireCondition(Array.isArray(input.spacingM) && input.spacingM.length === 3 &&
    [0, 1, 2].every(i => Object.hasOwn(input.spacingM, i) && input.spacingM[i] === SPACING[i]) &&
    input.exterior === 'closed',
    'actual voxel metric and explicit closed exterior policy required');
  const soils = definitions(input.definitions), cells = soilNodes(input.cells, soils);
  const lookup = new Map(cells.map((n, i) => [key(n.at), i]));
  const reservoirDefs = reservoirInputs(input.reservoirs), ports = compilePorts(input.ports, lookup, reservoirDefs);
  const nodes = Object.freeze([...cells, ...reservoirNodes(reservoirDefs, ports, cells)]);
  requireCondition(nodes.length <= REGION_LIMITS.maxUnknowns, 'bounded shared pressure unknowns');
  const compiled = compileFaces(cells, nodes, lookup, ports, input.closedFaces);
  const topology = stableTopology(nodes, compiled.faces);
  const descriptor = Object.freeze({ version: 'rigid-soil-voxel-graph-v1', regionId: input.regionId,
