...
import { createWorldSpec, sampleCell, sampleTerrain } from '../height-sea/terrain.js';
import { drainage } from './drainage.mjs';

const out = new URL('./run-v3/', import.meta.url);
...
try {
  ...
  check('bounded marine input and numeric capacities reject overflow before analysis', () => {
    assert.throws(() => build(Array(9).fill(0), 3, 3, Array(10).fill(0)), /root indices/);
    const extremes = Array(9).fill(1e308); extremes[4] = -1e308;
    assert.throws(() => build(extremes, 3, 3), /representable/);
    const span = Array(9).fill(1e308); span[4] = 0;
    assert.throws(() => build(span, 3, 3), /representable/);
    assert.equal(build(Array(9).fill(0), 3, 3).describe().typedArrayAllocationBytes, 46 * 9);
  });
  ...
  for (const fixture of [
    ...
    for (let row = 0; row < height; row++) for (let col = 0; col < width; col++) {
      const x = fixture.origin[0] + col * fixture.stride, z = fixture.origin[1] + row * fixture.stride;
      // Physical bed queries need no four-neighbor display facts.
      const cell = sampleTerrain(spec, x, z, 1);
      if ((row * width + col) % 257 === 0) {
        const display = sampleCell(spec, x, z);
        assert.equal(cell.bedMetres, display.bedMetres);
        assert.equal(cell.terrain, display.terrain);
      }
    ...
    const prior = await readFile(new URL(`./run-v1/${fixture.name}.json`, import.meta.url), 'utf8');
    const frozen = JSON.parse(prior);
    assert.deepEqual(payload.samples, frozen.samples, 'same actual bed/drainage fields after boundary/refactor correction');
    assert.deepEqual(payload.components, frozen.components, 'same actual component capacity records');
