...
import { createWorldSpec, sampleCell, floorDiv, mod } from '../height-sea/terrain.js';

const RECIPE = Object.freeze({
  codecSchema: 2,
  heightSource: '530464448725cacb73836f34c2c48ddbf3e4a8c0d8a353498e7fe6da03f45bdf',
  voxelRecipe: 'height-sea-cave-density-v2',
...
function caveLattice(fieldHash, x, y, z, salt) {
  return hashString(`${x}|${y}|${z}|${salt}`, fieldHash) / 0xffffffff;
}
...
export function createVoxelWorld(identity, { checkpoint = null, maxResidentBricks = RECIPE.residentBrickCap } = {}) {
  ...
  const fieldIdentity = `${spec.identity}:${RECIPE.voxelRecipe}:${RECIPE.cave.id}`;
  ...
  function columnAt(x, z) {
    // Exactly one corrected height-sea oracle call per generated brick column.
    const cell = sampleCell(spec, x, z); heightSamples += 1;
    return { bedLevel: cell.bedLevel };
  }
  ...
  function baseMaterialAt(at, column = columnAt(at.x, at.z)) {
    ...
    if (at.y >= bed) return MATERIAL.air;
  ...
  return Object.freeze({
    ...
    describe() {
      return {
        ...
        coldBrickBudget: { cells: VOLUME, heightColumns: SIDE * SIDE, caveMetricsAtMost: VOLUME },
