...
export function createVoxelWorld(identity, { checkpoint = null, maxResidentBricks = 8 } = {}) {
  ...
  if (checkpoint !== null) {
    ...
    for (const entry of checkpoint.changes) {
      const at = checkCell(entry); checkMaterial(entry.material);
      integer(entry.revision, 'change revision');
      if (entry.revision < 1 || entry.revision > checkpoint.revision || changes.has(cellKey(at)))
        throw new Error('duplicate or invalid change revision');
      putChange({ ...at, material: entry.material, revision: entry.revision });
      latestRevision=Math.max(latestRevision,entry.revision);
    }
    if (checkpoint.revision !== latestRevision)
      throw new Error('checkpoint frontier has no corresponding change');
  ...
  function decode(at) {
    ...
    for (let z=0; z<SIDE; z++) for (let x=0; x<SIDE; x++) {
      ...
      for (let y=0; y<SIDE; y++) {
        const level = y0+y;
        material[(y*SIDE+z)*SIDE+x] = level >= surface ? MATERIAL.air : level >= surface-2 ? MATERIAL.soil : MATERIAL.stone;
      }
    }
  ...
  return Object.freeze({
    ...
    edit({ expectedRevision, cells }) {
      ...
      for (const input of cells) {
        ...
        next.push({ ...at, material, revision:revision+1 });
      }
      ...
      for (const entry of next) {
        putChange(entry);
        cache.get(brickKey(entry))?.set([entry.material],index(entry));
      }
      revision++;
      return { ok:true, revision, changedBricks:[...new Set(next.map(brickKey))].sort() };
    },
    save,
