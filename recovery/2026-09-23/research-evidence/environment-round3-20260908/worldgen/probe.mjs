...
const directory = new URL(`./${process.argv[2] ?? 'run-v3'}/`,import.meta.url);
...
try {
  ...
  await check('world, source recipe and unit mismatches reject restore',()=>{
    ...
    for (const edit of [s=>s.identity.base.generatorSource='changed',s=>s.identity.base.units.verticalMetres=1,
      ...
      s=>s.changes[0].material=MATERIAL.stone]) {
    ...
    assert.throws(()=>worldIdentity({worldId:'bad-seed',seed:{}}));
    const reordered=Object.fromEntries(Object.entries(identity).reverse());
    assert.deepEqual(createVoxelWorld(reordered,{checkpoint}).save(),checkpoint);
    const mutable=structuredClone(identity), isolated=createVoxelWorld(mutable);
    mutable.base.units.verticalMetres=999; assert.equal(isolated.save().identity.base.units.verticalMetres,.54);
  });
  await check('no-op and return-to-base keep only real sparse differences',()=>{
    const fresh=createVoxelWorld(identity,{maxResidentBricks:1}),at={x:-2048,y:-64,z:-2048};
    assert.equal(fresh.read(at),MATERIAL.stone);
    assert.deepEqual(fresh.edit({expectedRevision:0,cells:[{...at,expectedMaterial:2,material:2}]}),
      {ok:true,revision:0,changedBricks:[]}); assert.equal(fresh.save().changes.length,0);
    assert(fresh.edit({expectedRevision:0,cells:[{...at,expectedMaterial:2,material:0}]}).ok);
    assert(fresh.edit({expectedRevision:1,cells:[{...at,expectedMaterial:0,material:2}]}).ok);
    const saved=fresh.save(); assert.equal(saved.revision,2); assert.equal(saved.changes.length,0);
    assert.equal(fresh.stats().changedBricks,0); fresh.evictAll();
    const restored=createVoxelWorld(identity,{checkpoint:JSON.parse(JSON.stringify(saved))});
    assert.equal(restored.read(at),MATERIAL.stone); assert.deepEqual(restored.save(),saved);
  });
...
for (const file of ['voxel-world.mjs','probe.mjs','reference/terrain.js','reference/worker.js']) {
  ...
  report.hashes[file]=createHash('sha256').update(source).digest('hex');
  if (!file.startsWith('reference/')) await writeFile(new URL(file,directory),source);
}
