...
export const WORLD_LAB_SPEC = Object.freeze({
  ...
  terrain: Object.freeze({
    ...
    quantization: Object.freeze({ kind: "round-normalized-height-times-32", steps: 32, minimum: 0, maximum: 32 }),
    seaSurfaceLevel: 12,
    coastShaping: Object.freeze({ transitionWorldUnits: 96, waterwardOffset: -0.10, landwardOffset: 0.08 }),
    surfaceWaterInitialization: "all-below-datum-surface-basins-at-waterline-v1",
  }),
});
...
export function quantizeBedLevel(spec, elevation) {
  ...
  return Math.round(
    Math.max(0, Math.min(1, elevation)) * quantization.steps,
  );
}
...
function coastHeightOffset(spec, coastDistance) {
  ...
  const raw = Math.max(
    0,
    Math.min(1, (coastDistance + shape.transitionWorldUnits) / (2 * shape.transitionWorldUnits)),
  );
  return shape.waterwardOffset +
...
export function sampleTerrain(spec, x, z, footprint = 1) {
  ...
  // The old hard coastDistance sign branch jumped .18 normalized height at the
  // shaping line.  Coast remains a landform input, but this blend is continuous.
  ...
  const surfaceWaterPotentialDepthLevels = Math.max(0, seaSurfaceLevel - bedLevel);
  // Classification is solely the final quantized bed versus the declared datum.
  // Surface-water is initialization metadata, not water stock or connectivity.
  // Equality describes dry ground at the sea datum. It is not a shoreline:
  // shore requires an authoritative neighbouring wet/dry relation below.
  const terrain = bedLevel < seaSurfaceLevel
    ? "surface-water"
...
export function namedFeatures(spec) {
  ...
  return {
    ...
    wetDryBoundary: nearestWetDryBoundary(spec, coastX, Math.round(coastLine(spec, coastX))),
  };
}
...
function exactWetDryBoundary(spec, cell) {
  ...
  const neighbours = CARDINAL_NEIGHBOURS.map((offset) => {
    const neighbour = sampleTerrain(spec, cell.x + offset.x, cell.z + offset.z, 1);
    return {
...
// A named jump is a bounded search for a real adjacent wet/dry pair.  The old
// shaping line only seeds this search; it cannot name a coast by itself.
export function nearestWetDryBoundary(spec, anchorX, anchorZ, maxDistance = 1024) {
  ...
  if (maxDistance < 0) throw new RangeError("wet/dry boundary maximum distance must be nonnegative");
  ...
  throw new Error(`no wet/dry boundary within ${maxDistance} cells of ${anchorX},${anchorZ}`);
}
    ...
    sampleRows(rowCount) {
      ...
      for (let row = nextRow; row < endRow; row += 1) {
        for (let column = 0; column < width; column += 1) {
          ...
          moisture[index] = Math.round(cell.moisture * 255);
          if (cell.feature)
            featureCounts[cell.feature === "coast-shaping" ? "coastShaping" : cell.feature] += 1;
        }
      }
      nextRow = endRow;
      return {
